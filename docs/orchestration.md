...
**Handing a busy colleague a second task is normal, and it says so.** Each `agent` call is its own request with its own id: the reply tells you whether it `started`, `injected` into the turn already running, or `queued` (with its 1-based position), and `agent_read{sid}` lists what that session still owes and to whom. A completion answers exactly the request it belongs to, and a turn none of your requests is bound to — the child's own human asking it something — answers none of them: your task keeps waiting instead of being closed by an answer that was never yours. So a child that looks silent is telling you *why*: it is working, or your task is third in line — never the same flat "pending" for both.
...
|---|---|
| "**hand codex** the RFC-12 implementation — work in the background, report back with a diff summary and test results" | a codex session grinds in the background; ONE notification when the task really completes — however many turns of its own the child needed — then you review the diff yourself with `git diff` |
...
One trade-off worth knowing about the steer: on Claude the steering line is shown to the model as a mid-turn preview, and the model usually answers it right there, inside the turn it joined; the CLI may instead **re-run** it as the next prompt. ccteam cannot see which happened, so when the joined turn ends it holds that boundary for a short grace (30 seconds): if a replay turn opens, your request follows it and that turn's receipt is your notification; if nothing opens, the joined turn's boundary is the answer and you are notified then. Steer when you are correcting work you can see; ask for a queued turn when you want a separate, quotable answer to the new task.
...
8. **Ask for the verdict on the FIRST line** (`VERDICT: ship` / `VERDICT: blocked — …`). The completion notification excerpts from the top of the answer, so a report that buries its conclusion under a preamble makes you open the transcript to learn what you already asked for.
...
3. **Collect when each task really ends** — one completion notification per task, at the boundary that ends the work; `agent_read{sid}` picks up anything you're still missing (an absent or failed member is noted, never killed).
   **A Claude child answers in as many turns as its own work needs.** Claude Code wakes its own model: when a background task, a `Monitor`, or an `Agent` the child launched finishes, the CLI prompts it again and the model answers in a fresh turn ccteam never submitted. So a boundary ends your task only when the harness is holding nothing that will wake the child again — no background task left in its snapshot, no injected line the CLI might still re-run as the next prompt. Until then your request stays bound (the turn the vendor opens next inherits it), collects one `progress` row naming that execution turn — visible in `agent_read`'s request rows — and **nothing is pushed to you**, neither `brief` nor `final`, though every one of those turns is billed. The two holds differ in how long they last. An injected line is the weaker one: if no replay turn opens within 30 seconds of the boundary, that boundary is delivered as the answer. A background hold has no timer — but it cannot outlive the child's process: when the child is released (idle or capacity) or the daemon restarts, the last boundary it held open is delivered as the answer, because nothing can wake it any more. The child reads `working` in between (`stale` after five silent minutes, the ordinary quiet-session word), and an `agent_stop` still cuts the chain wherever it has got to. You get exactly one notification, and it points at the turn that actually carried the answer. On codex and the ACP vendors a turn exists only because ccteam asked for one, so every boundary there is already the end.
   Every completion notification is one header line — `s12 done · codex · turn 7 · ctx 19% · «the verdict» req-18d2… · 2 still queued` (`⚠` from 85%; `s12 FAILED (<kind>) …` on failure) — followed by the answer excerpt (500 characters by default, 2000 under `notify:"final"`): the whole answer when it fits; otherwise the child's **conclusion** — what it wrote after its last tool call, the receipt a disciplined worker ends on — behind one `…[+N chars: agent_read{sid,turn:<turn_id>,max_chars}]…` marker that names the exact read of that answer (it stays correct after the child finishes later turns); only a turn with no such boundary falls back to a head/tail preview. The header names **which** of your tasks came back and how many that child still owes you, so several tasks on one child never blur into one report. If you are mid-turn when children finish, Claude parks their notifications and delivers the ones already queued at the next boundary in one batch turn, without waiting to gather more. Every item's header and excerpt stay verbatim; requests are recorded independently. Batches target 50,000 characters including replay markers, split in FIFO order, and state how many notifications remain. A single indivisible oversized notification is labelled and delivered alone with its text intact. Explicit queued tasks and slash commands remain separate turns. A batch retained across a crash replays with the same identity and a repeat marker, including lines already read through `agent_read` (`collected` is not a consumption acknowledgement). Delivery is at most once while the daemon lives (a crash mid-delivery can repeat it in the next life rather than drop it: same `request_id`, same turn, same completion); and `agent_read` rows, transcripts and inline `agent` results carry `context_pct` as a number, so you can decide **reuse vs. fresh session** without another call: keep dispatching to a child while its context is comfortably low; once it nears the warning band, spawn a new one for the next task and let the old one idle. Mid-turn narration never wakes you at all — it stays in the child's ledger — so a chatty child costs you nothing extra, and when you do want to LOOK, `agent_read{sid,n:0}` answers `partial:true` with a bounded excerpt of what the running turn has said so far: a way to see what a working child is doing that is not stopping it to find out. If a child outlasts an inline `agent{wait}`, `agent_read{sid,wait:240}` holds the read open until its turn ends instead (repeat with `since:<cursor>` to page forward — oldest unread first, `remaining` says how many are still unread); `agent_read{sid,n:0}` omits completed turn bodies while still reporting running narration. For repeated reads, keep the returned `cursor` and send `since:<cursor>`: only newer completed turns are returned, plus the current `in_flight` excerpt. `n:1` reads the latest completed turn and may repeat it; `turn:<id>` locates one exact turn and is not a deduplication rule. `max_chars` is shared by serialized `turns`, `requests`, and `in_flight`; small control fields and omission counts sit outside that payload budget. Completed request history is available with `history:true`. None of this is polling: for your own hires the completion notification is authoritative, and reading a child's `turns.jsonl` yourself is never necessary.
...
**The bill stays visible.** `agent_read` rows carry the accrued `cost_usd` / `tokens_total` per member, so a fan-out's cost is a sum you can read, not a surprise. One honest caveat for **codex** members: the app-server reports token usage per Responses request, not per turn, and a turn is usually many requests — so a codex member's `tokens_total` and `cost_usd` are a **lower bound** on what the turn actually burned, not an exact figure. Every other vendor reports per turn. Treat a codex number as "at least this much".
...
| "it's been silent forever" | it's **working**, not stuck — and a Claude child that started background work of its own answers across several turns before its task ends, staying `working` between them. Go do something else and come back for the report. |
...
| "it's been quiet for half an hour — is it lost?" | look, don't stop it. `agent_read{sid,n:0}` answers `partial:true` and `in_flight{turn_id, narration, text}`: the tail of what the turn currently running has said (public narration only, never its private reasoning), plus which of your tasks that turn is answering. Reading it changes nothing and resolves nothing — the completion still arrives when the turn ends. `narration:"unknown"` means that harness cannot report a running turn's words, which is not the same as silence. |
...
- **Notifications are batched at a boundary, never merged; no line is dropped, deduplicated or reordered.** An explicit stop does not cancel a queued task, and ccteam never re-sends one on your behalf either.
- **Storage errors preserve queued input.** An unreadable `pending_turns.jsonl` row blocks the whole drain and reports an error; the file stays intact for repair. A failed notification write-ahead pauses delivery until another boundary or resume can retry. A crash before durable retirement may replay a consumed batch, with its repeat marker and original remaining-count snapshot.
...
Moved to its own page: **[mcp.md](mcp.md)** — the complete reference for all six tools (`agent` · `agent_read` · `agent_stop` · `status` + its `grok_claude_codex_kimi` alias · `chat_send_file`): parameters, response shapes, the per-caller tool face, notification tiers, credential families, protocol details, and the delegation guardrails. You normally never need it — your session drives the tools from your plain-language ask — but skill authors and manual orchestrators read it once.
