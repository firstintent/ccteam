...
| per-session web 历史 / SSE / 审批渲染 | 历史 = `crates/ccteam-web/src/routes/sessions_api.rs`(`GET /sessions/{sid}` → `session_resolve` → `collect_session_turns` 读 `turns.jsonl`;按 sid 过滤 SSE `event_matches_sid`);SPA = `web/src/pages/ChatConsole.tsx`(`/chat/s/:sid` keyed)+ `lib/sessionsApi.ts` + `hooks/useSessionEvents.ts` + `pages/chatTranscript.ts` | `cargo test -p ccteam-web sessions_api`;vitest |
