...
function NoticeRow({ notice, t }: { notice: ChatNotice; t: T }) {
  if (notice.kind === 'queued') {
    return (
      <div className={`${css.notice} ${css.noticeQueued}`}>
 ...
 */
export function SessionChat({ sid, chat, node, store, api, t }: SessionChatProps) {
  ...
  useEffect(() => {
    let alive = true
    store.dispatch({ type: 'history_loading', sid })
    api
      .call('session.history', { sid })
      .then((history) => {
        if (!alive) return
        store.dispatch({ type: 'history_loaded', sid, rows: history.rows })
      })
      .catch(() => {
        if (!alive) return
        store.dispatch({ type: 'history_failed', sid, message: t('chat.history.error') })
      })
    const stream = api.events(
      {
        onEvent(event) {
          if (event.kind === 'session') {
            if (event.row !== undefined) store.dispatch({ type: 'event_row', sid, row: event.row })
            if (event.activity !== undefined) store.dispatch({ type: 'activity', sid, activity: event.activity })
            return
          }
          if (event.kind === 'turn_done') store.dispatch({ type: 'turn_done', sid })
  ...
  const send = (): void => {
    ...
    store.dispatch({ type: 'send_started', sid, text })
    api
      .call('session.send', { sid, text })
      .then((receipt) => {
        store.dispatch({ type: 'send_settled', sid, receipt })
      })
      .catch((error: unknown) => {
        store.dispatch({ type: 'send_failed', sid, message: error instanceof Error ? error.message : String(error) })
      })
  }
    ...
    <>
      ...
      >
        <div className={css.transcript}>
          {chat.error !== null && <div className={`${css.notice} ${css.noticeError}`}>{chat.error}</div>}
