...
| `flows/` | ccteam **Flow** scripts (deterministic JS driving real cross-harness hires) | `ccteam flow run examples/flows/audit-fanout.flow.js --project <slug>`; start your own with `ccteam flow new <name>`, which scaffolds into `.agents/flows/` (git-shared — **not** `.ccteam/`, which ccteam gitignores) and prints the script surface |
...
**Improving a flow.** `flows/flow-review.flow.js` grades a finished run from its journal and proposes edits, both as validated JSON. Install it once as the evaluator — `cp examples/flows/flow-review.flow.js .agents/flows/_eval.flow.js` — and `ccteam flow eval <run-dir>` finds it. `flows/self-review-loop.sh` wires write → run → evaluate → improve into one recipe; it stops at the improve hop, because script space has no filesystem and a flow cannot edit itself.
