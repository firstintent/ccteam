...
| 项 | 值 |
|---|---|
| Workspace version | `0.9.13`(**发布中** 2026-08-08:owner「拉最新 dev,走 release 流程,正式发布」;ship gate 已回填,dev→main PR + tag + release 见 `.loop/state.md`「版本线」;下版 tag 仍需 owner 显式发话) |
| 本版 headline | **Pi = 第六 vendor**(`pi --mode rpc` 长驻 stdio JSONL 自有协议;工具面 = **受管会话 bridge**(ccteam-owned 扩展在 spawn 时挂载)故**不写 pi 任何配置**、手起的 `pi` 一动不动 —— `AgentProbeSpec.tool_surface = ManagedSessionBridge` 让这条差异从注册表里长出来而非散在分支;role 支持(用户 `.claude/agents/<role>.md` **body 原文**投射 + sha,frontmatter `pi.model` 须 canonical `provider/model`)· HITL 走共享 approve/deny 面 · 强度轴**按模型**自报 · 计价只认 vendor 自报 USD · `HostExecutionScope::LocalOnly` 把「本机 only」写进类型、跨机门对每个 vendor 穷举);**paneless turn 全生命周期进 `progress.jsonl`**(只镜像 turn 末尾 → 外部读者把活会话读成 idle、`collect(tail)` 把上一轮答案当新答案;三边界补齐:开 = `after_turn_submitted` 唯一 choke point · 心跳 = 60s 节流按 `is_terminal()` 认族(新 paneless harness 落地即覆盖)· 关 = 三个终结事件都落行)+ **失败 turn 照样入账**(跨 vendor 携带 usage/model —— 失败计费,零成本是谎)+ schema **净减**:`chat_turn_failed` 整体退役,一个失败 turn 恰一条终结行;**后台任务归属改认 vendor 快照**(claude `Agent` 已异步化,硬编码 `task_type` 表把每个异步 agent 都清了 —— 改消费 `system:background_tasks_changed` 全量快照,快照只**收窄**安全网永不停用它)+ `/status` 静默基线取 `max(turn 开始, 最后事件)`(刚发出的 turn 不再秒读 STUCK);**受管 grok 不再继承 Claude 插件的 MCP server**(每个受管 grok 拖一个 `bun …/telegram` 抢用户 bot 的 `getUpdates` —— 六个 compat env / 两级 `[plugins].disabled` / `CLAUDE_CONFIG_DIR` 真机全证伪,有效解 = `--plugin-dir` 同名空插件遮蔽,判据「声明了 MCP server」故新插件零改码被覆盖,全程 fail-open);web 四改(sidebar 工作区 **⋯** 菜单 = 项目移除/新建会话/复制路径,回到项目所在处 · Access「我的 IM bot」每平台一张**两步引导卡**(独立保存 + 保存后自动开始捕获发送者)· Ops 每主机 **harness 管理卡**(整份 vendor 清单 + 只在后端真会接受写入处给按钮)· chat markdown 表格不再撑破气泡)+ 中立 `routing.md` 起手模板(删预设角色/模型样表,零分工意见) |
...
| **session 调度门 = daemon 校验 per-session principal `(sid,secret)`(best-effort,非硬边界)** | 5 个 `session_*` 工具由 daemon 按 principal 校验(**任意有效 session**):spawn 时 mint per-session secret,受管会话默认走 HTTP bearer `ccteam-sid:<sid>:<secret>`;`verify_session_principal` 常数时比对,caller 的 project **服务端覆写**(不信自报,只能操作自己 project);**授权只认 principal,任何自报属性都不参与**;`dispatch`/`stop` 是显式调度(非主动 kill),stop 限后代 + depth/children/delegated/cycle/预算护栏。**主会话 fallback**:非 ccteam 拉起的本机会话按 vendor 全局注册以 Admin 语义服务(HTTP admin bearer;Claude stdio forwarder 读 0600 `web-token` = 同 uid 证明)。**诚实范围**:单 OS-uid 全信任模型下 agent 间**无硬边界**(同 uid 可读他进程 env/文件/ptrace → 拿 secret),secret 只**抬门槛**(defense-in-depth)**不 close**;真隔离 = per-agent OS user/sandbox(deferred) |
