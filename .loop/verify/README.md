...
- **env-flake 族**(live-daemon 宿主才出现,不计入 baseline;干净环境应全绿):
  ...
  (2026-08-18 v0.10.3 A/B 收口再实锤:`gateway_resumes_dead_session_on_next_turn` 在本机 `make test-baseline` 全并行下已**近确定性红**(pristine origin/dev 双复现 + dev 会话 3/3),隔离恒绿;已升级立卡 TEST-HYG-1,勿再仅登记。)
