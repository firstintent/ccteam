...
- **偏差**:①SPA composer 菜单的 mode 选择面**未做**(API 层已通:`sessionsApi.mode` 透传)——UI 设计活独立成卡 DSH1-MODE-UI;②additive 契约变更(MCP `session_spawn.mode` + REST form `mode` + meta `mode` 字段)依据 = owner 2026-08-18「按照四种模式,默认走 PTC」直令。
...
- **规格**:composer 的 model 菜单在 vendor=dsh 时增加第四段「模式」(标准/PTC(默认)/极简/创造 → standard/ptc/minimal/creator),draft 增 `mode`,`createSession` 已收 `mode`;非 dsh vendor 不渲染该段。
