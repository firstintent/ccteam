...
| 会话状态行(per-turn,IM / web / MCP 三端同源)| 一个 struct + 一个渲染器 + 一个采集点:`crates/ccteam-harness/src/execution/turn_status.rs`(`TurnStatus{model,context,turn,cost_usd,tokens_total}` + `StatusIdentity` + 唯一文本渲染器 `render_status_line`,字面 `→ slug/sid (role) · vendor model · ctx N%⚠ · turn N · $x.xx 「title」`,缺字段整段省略);采集点唯一 = `ccteam-im/src/gateway.rs` `spawn_event_pump` 落 assistant 记录处(`thread_status()` + 会话账本累计,不扩 `CanonicalEvent`);携带面 = `TurnRecord.status`(turns.jsonl)/ `GatewayEvent.status`(web SSE `session_event_payload` + history `turn_to_event`)/ IM 回复尾行与 `/status` 头行 / `delegation.rs` 完成·interim 通知第二行 / `session_collect|dispatch|spawn` 信封 `status_line` + `session_list` 行 `context_pct`;各前端只携带不自算 | `cargo test -p ccteam-harness turn_status`;`cargo test -p ccteam-im turn_answer_carries` |
