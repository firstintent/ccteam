...
- **Settings from a LAN browser:** DSH treats its settings document (every Settings page, including Plugin configuration) as a privileged surface for the operator's own machine, and its client decides that from the page address — a native `dsh web` opened over a LAN address shows an empty Plugin configuration tab and keeps settings read-only. Through the companion listener that stand-in is your ccteam identity: the instance is yours alone and ccteam already authenticated the page, so the listener declares the page owns its Host through DSH's own transport hook (`__DSH_TRANSPORT__.ownsHost`) and settings work from any browser. DSH reads that hook natively from the release after 0.1.1-rc.2 (it is on DSH main); for rc.2 and earlier, whose client decides from the page hostname alone, the listener back-ports that one read into the client-connection bundle it serves (an exact-line rewrite that is a no-op on any other version), so settings work from any browser on rc.2 as well. One consequence: DSH's deliverable file actions then act on the daemon host — the workspace machine.
...
For the opposite direction — installing ccteam's plugins into a `dsh web` you run yourself, and using the ccteam workbench inside DSH’s own console — see [dsh-plugin.md](dsh-plugin.md) ([中文](dsh-plugin-cn.md)).
...
### Team View: Topology, Charter and Runs

The **Team** page is the multi-vendor cockpit, three tabs:
...
- **Runs (编排)** — every `ccteam flow run` the daemon recorded on the ledger, newest first across your visible projects: name, description, status (running / done / error / brake — a brake is a guardrail refusing new hires, not a crash), agents hired, cost, duration; expand a row for the sessions it actually hired (real links) and the session that triggered it. With no runs yet the tab is the on-ramp: the three commands that start one, copyable. A flow is written and launched from a shell — or by any session — never from the web; see [hook-dynamic-workflows.md](hook-dynamic-workflows.md).
...
Any agent ccteam does not manage (your own script, a hand-started CLI, an agent on another machine) can call the daemon's MCP endpoint directly with an **enrollment credential** and get the same six tools a managed session has:
...
- **Two scopes.** Every `ccteam daemon start` writes the **user-scoped** machine credential — the unlabelled slot, reused across restarts, never re-minted — into the vendor configs on that machine, so a hand-started Claude/Codex/Grok/OpenCode/Kimi already has the tools; DSH gets the same identity model through its `@ccteam/ccteam-ui` plugin, which asks the local daemon for its own user-scoped credential (labelled `dsh-plugin:<profile>`) or takes a pasted one for a daemon elsewhere. Because a user-scoped credential names no project, such a caller must pass `project` explicitly on its first `agent` call, and only its owner's projects are accepted. A **project-scoped** credential is what the console's copy button hands out (Settings → Access, or `POST /api/v1/projects/{slug}/enroll`): pinned to one workspace, safe to paste on another machine, listed and revocable afterwards. The secret is shown once, at mint time.
- **Ensure, not mint, for programs that boot repeatedly.** `POST /api/v1/enroll` with `ensure: true` and a `label` is idempotent per (identity, label): `201` with the bearer when the daemon creates the record, `200` with only `bearer_prefix` when it already exists — the secret is never returned twice; a caller that lost its copy sends `rotate: true` to replace the record instead of adding a second one. The console's own mint (no `ensure`) still creates a fresh credential every time.
...
Every session can hire colleagues, hand them tasks, and read their answers through the `mcp__ccteam__agent` / `mcp__ccteam__agent_read` tools — no manual switching, no special role. Just ask in natural language:
...
A session deciding whom to hire never has to guess. `status` (the MCP tool, also exposed through the byte-identical discovery alias `grok_claude_codex_kimi`) answers in tiered compact JSON. The default `brief` tier is the hiring answer and little else: which vendors the host your current project is bound to can actually hire, the team's cost over the last 24 hours, and — only when they apply — an offline-or-stale flag for a satellite host and any vendor a budget cap has disabled. `detail` buys the rest, and only for the call that asks: `models` adds the observed model ids and reasoning-effort ladders per vendor plus the hub `models.json` catalog, kept as two separately labeled sources that are advisory and never a hiring allowlist; `vendors` adds installed / version / an honest auth signal (sitting on PATH never masquerades as logged in) / budget posture per vendor with an observed-at; `routing` adds your **routing notes** transported verbatim (source, sha256, truncation flag), or the paths it looked at when you have none; `full` adds daemon health and every visible project's 24h cost on top.
...
ccteam doctor --verify-mcp         # MCP surface check: 6 tools / 0 stubs; drift exits 1.
```
...
Remote execution currently supports Claude stream-json sessions; the connection self-heals with backoff, and a dropped exec link resumes context via vendor `--resume` on the next spawn. Pi and DSH are local-only by design — their sessions run on the daemon's own machine, and a project bound to a satellite refuses those spawns with a plain error rather than quietly running them somewhere else. Fleet capacity: ccteam releases a session's harness process once nobody has spoken to it for `sessions.idle_release_secs` (default 3600 — one hour, matching Claude's prompt-cache TTL, so the process is held exactly as long as holding it can still save a cache read; `0` never releases, and `sessions.idle_release_by_vendor: {claude: 7200, codex: 0, …}` overrides it per harness). A released session is NOT gone: it keeps its transcript, its place in `/sessions` (marked 💤), and its chat's focus — the next message resumes that same sid in about a second. Nothing is released mid-turn, while it waits on an approval, or while its harness reports a background task still running. `sessions.max_live` (default 50) remains as a hard ceiling for bursts: admitting one more past it gracefully releases the least-recently-active eligible session. Only `/stop` ends a session for good.
