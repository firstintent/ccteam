...
- **`max_chars`** 默认 1000(100–50000),由 `turns` + `requests` + `in_flight` 三个 JSON 值共享,元数据、标点、转义全部计入;状态、游标、省略计数和等待结果控制字段在预算外。短内容把余额留给长内容。turn 截断优先保留结论,否则头尾节选;完整读取指针都放不下时,可用返回的 `turn_id` 配合 `turn` 和更大预算精确读。`truncated:true` 明示截断。整行放不下就不返回,向前游标绝不跳过未返回行;全文仍在账本。
