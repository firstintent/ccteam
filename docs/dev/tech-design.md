...
| **compare + curated MCP + evolution**(v0.8.24 C)| compare = `ccteam-im/src/compare.rs` + gateway `run_compare_for_web` + REST `routes/compare.rs` + IM `/compare`· curated MCP = `execution/mcp_config.rs` + stream-json spawn 写 `chat/<sid>/mcp.json`· evolution = `routes/evolution.rs`(读 `experience.jsonl`)| `cargo test -p ccteam-im --lib compare`;`cargo test -p ccteam-web --test hosts_multihost_test`(hosts)+ openapi |
