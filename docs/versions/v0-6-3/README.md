...
|---|---|---|---|---|
| **F140** | `Trigger::Schedule` 接真 cron(收尾 V0.4.6 stub) | 9 + 14 | §3.3.2 | S |
| **F141** | Webhook ingress(HTTP→文件入口) | 9 + 14 | §2.1 / §3.8 | S |
| **F142** | vendor 接缝 forward-compat 解析 | 14 | §2.1 HarnessAdapter | S |
| **F143** | 跨 session 运行时路由(squad 缩水版) | 13 | §3.3 | M |
...
- **F140 / F141** —— cron 与 webhook 只有「24/7 醒着的 daemon」能做,harness session 结构上不能给自己定时 / 监听外部事件;
- **F142** —— ccteam 的本分就是读 sub-harness 吐出的东西,vendor 按自己节奏发版,「读那条缝别崩」是 meta-harness 承重关节;
- **F143** —— 单 session 内的委派已被 Claude Code `Task` subagent 解决、ccteam 早已下沉;只补「跨 spawn 运行时路由」这条窄缝。

**被筛掉的:** Multica 的 per-issue KV store —— 真 meta-harness 地盘但不痛(事件→`progress.jsonl`、交付物→artifact 已覆盖)+ 有「第四 SoT」风险,撞墙再说。

---

## 1. F140 — `Trigger::Schedule` 接真 cron
...
## 2. F141 — Webhook ingress(HTTP→文件入口)
...
## 3. F142 — vendor 接缝 forward-compat 解析
...
**与「不做历史迁移」红线的关系(重要):** 不冲突。重装红线管的是 **ccteam 自有 + 升级时一起 wipe** 的数据(workflow.yaml / ccteam 自有 state.json / progress.jsonl —— 升级即清,无跨版本老格式)。F142 管的是 **vendor 自有、ccteam 清不掉** 的文件。两类互不重叠。
...
## 4. F143 — 跨 session 运行时路由(squad 缩水版)
...
**设计哲学(为什么是「缩水版」):** Multica 的 Squad = leader 拆活分活。但「拆到能装进一个 context 的活」Claude Code 的 `Task` subagent 已在单 session 内做了,ccteam **早已正确下沉、绝不重实现**。真正剩下的 meta-harness 窄缝只有「跨 spawn / 跨 session 运行时路由」;而 mode-3 的 bot-to-bot `@routing` + `hop_limit` 已验证此模式 —— F143 只是把它搬进 mode-2 bg workflow。
...
**并行 / 串行约束:**
- **F141、F142 完全独立,立即并行**(不碰 workflow schema)。
- **F140 与 F143 都改 workflow.yaml schema**(`AgentSpec` 加 `schedule` / 顶层加 `squad:`)→ schema 改动串行:**F140 先落 schema 并 merge,F143 基于其上加 `squad:` 块**(或一人统一改 schema)。
- 推荐排期:Wave A = F140 + F141 + F142 并行;Wave B = F143(F140 schema merge 后起)。
...
- `dev-coupling-audit.md` 加 F140-F143 索引。
