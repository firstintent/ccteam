...
**Gateway real-path knobs**:`CCTEAM_HOME` overrides the daemon ccteam root for gateway state/outbound ledger/config lookup, letting real WS smoke tests isolate ccteam-owned state while preserving `$HOME` for Claude/Codex auth. In daemon mode, the gateway installs an async event sink: harness `ThreadEvent` assistant/error text is converted to outbound `SendMessage` rows and delivered through the durable ledger after the immediate `submitted <session> turn <id>` ACK. Without an event sink (direct unit tests), `CCTEAM_IM_GATEWAY_REPLY_WAIT_MS` controls the synchronous first-event wait before that ACK; default is 5ms. `CCTEAM_IM_GATEWAY_SUBMIT_TIMEOUT_MS` bounds harness submit calls before returning a visible `gateway error: submit timed out ...`; default is 5000ms. `CCTEAM_REAL_IM_WS_NL=codex|claude|1` makes `scripts/smoke-im.sh --real` require true natural-language replies over the real WS dual-harness path.
