...
- **`session_list`** — 在线名册(忙闲、`context_pct`、成本/token、`parent_sid`),按最近活跃排序;`tree:true` 才返回过滤后各行的委派树。支持 `{project?, activity?, limit?, tree?}`(默认最多 30 行,只在截断时带 `truncated`/`total`;空字段省略),大船队不再灌爆你的上下文。web 团队视图渲染的是同一张图。
