...
- 已在 `docs/versions/v0-3-1/README.md` 顶加 erratum 行
...
- e2e：playwright happy path（dashboard → project → session → BTW →
  ...
  用 mock tmux
- 文档：`docs/versions/v0-3-2/user-manual.md` 写 SPA 用法 + 两条输入通道（WS PTY
  vs `/btw` 注入）说明
...
- `docs/versions/v0-3-2/prd.md §<F-num>`
...
`docs/versions/v0-3-2/dev-plan.md` 在 §3 决策 confirm 后写：
...
`docs/versions/v0-3-2/user-manual.md` 在 F58 ship gate 前写（参考 V0.3.1 user-manual
结构）。
...
- V0.3 PRD `docs/versions/v0-3/prd.md` §3.8 当前 web UI 设计 SoT
- V0.3.1 PRD `docs/versions/v0-3-1/prd.md` §10 deferred 列表
