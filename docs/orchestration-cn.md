...
| 你说 | 发生什么 |
|---|---|
| 「RFC-12 的实现**交给 codex**,后台跑,完了给我 diff 摘要 + 测试结果」 | 起一个 codex 会话在后台干;任务**真正**完成时来**一条**通知(它自己要跑几个 turn 就跑几个),diff 你自己 `git diff` 审 |
...
steer 有一处代价值得知道:在 Claude 上,这行纠正会以 turn 中预览的形式给模型看到,模型通常就在它并进去的那个 turn 里当场作答;CLI 也可能把它当下一个 prompt **重跑**。ccteam 看不出发生的是哪一种,所以并进去的那个 turn 结束时,它的边界会被先扣住一小段宽限(30 秒):这期间有 replay turn 开出来,你的请求就跟过去、以那个 turn 的收尾作通知;什么都没开,并进去的那个 turn 的边界就是答案、此时通知你。纠正你看得见的活就 steer;想要一份可以单独引用的新答案,就要一个独立排队的 turn。
...
3. **在每条任务真正结束时收集** —— 一条任务一条完成通知,落在结束这段活的那个边界上;还缺的用 `agent_read{sid}` 补(缺席/失败的成员标记出来,绝不 kill)。
   **Claude 子会话的一条任务,它自己需要几个 turn 就答几个 turn。** Claude Code 会自己叫醒自己的模型:子会话起的后台任务、`Monitor` 或它自己派出的 `Agent` 一跑完,CLI 就自己递一行进去,模型在一个 ccteam 从没提交过的新 turn 里作答。所以一个边界只有在 harness 手上再没有能把它叫醒的东西时,才算结束你这条任务 —— 快照里没有还在跑的后台任务,也没有 CLI 可能当下一个 prompt 重跑的注入行。在那之前,你的请求保持绑定(vendor 自己开的下一个 turn 继承它),每过一个边界记一条 `progress`、点名那个执行 turn(在 `agent_read` 的 request 行里看得到),而**什么都不推给你** —— `brief` 和 `final` 都不推;但这些 turn 照样计费。两种「扣住」的时长不同。注入行是弱的那种:边界之后 30 秒内没有 replay turn 开出来,这个边界就作为答案投给你。后台任务的扣住没有计时器 —— 但它活不过子会话的进程:子会话被释放(空闲或容量)或 daemon 重启时,它扣住的最后一个边界就作为答案投出,因为再没有东西能叫醒它了。中间子会话读作 `working`(整整五分钟没动静才转 `stale`,就是平常那个安静会话的词),`agent_stop` 依然能就地把这条链切断。你最终只收到一条通知,它指向真正承载答案的那个 turn。codex 与 ACP 系那边,turn 只因为 ccteam 派了活才存在,所以每个边界本来就是结束。
   每条完成通知只有一行头 —— `s12 done · codex · turn 7 · ctx 19% · «裁决» req-18d2… · 还排队 2`(`⚠` 从 85% 起;失败写 `s12 FAILED (<kind>) …`)—— 后面直接是答案节选(默认 500 字符,`notify:"final"` 是 2000):装得下就是整段答案;装不下则给子会话的**结论** —— 它在末次工具调用之后写的那段、守纪律的工人收尾用的回执 —— 前面带一个 `…[+N chars: agent_read{sid,turn:<turn_id>,max_chars}]…` 标记,写明被略去的叙述字数与读那条答案的精确调用(子会话之后再完成别的 turn,它依然指向同一条);只有 vendor 没标出这个边界的 turn 才回落到头尾预览。头里点名**哪一条**任务回来了、以及这个子会话还欠你几条,一个子会话上的多条任务就不会糊成一份报告。父会话仍在 turn 中时,Claude 暂存完成通知,在下一可投递边界把当时已排队的通知放进一个批次 turn,不等待凑批。每条头部与摘录原文保留,请求逐条独立记账;每批预算 50,000 字符(含重放标记),超限按 FIFO 确定性拆批,尾部说明还剩几条。单条不可拆通知自身超限时明确标记并独立投递,保留全文。显式排队任务与斜杠命令仍各自成 turn。崩溃后仍留在镜像中的批次按原身份重放并标明重复,即使其中某条已被 `agent_read` 读过也不删除(`collected` 不是消费确认)。同一 daemon 生命期内至多一次(送到一半崩溃,下一个生命会重发而不是丢掉:同一 `request_id`、同一 turn、同一条完成);`agent_read` 的名册行、transcript 与内联 `agent` 结果都给数字 `context_pct`,据此决定**复用还是新开**不用再多调一次:上下文还宽裕就继续派给它;接近告警带就为下个任务新开一个、旧的闲置。中途叙述根本不会叫醒你 —— 它只留在子会话自己的账本里 —— 所以话痨子会话不多花你一分钱;而你真想**看一眼**时,`agent_read{sid,n:0}` 会给 `partial:true` 加一段有界节选:正在跑的那个 turn 说到哪儿了 —— 一种不用把它停掉就能看见它在干什么的办法。子会话比内联 `agent{wait}` 还久时,用 `agent_read{sid,wait:240}` 把这次读挂住到它的 turn 结束(带 `since:<cursor>` 重复调用即可继续向前翻 —— 最旧的未读在前,`remaining` 告诉你还剩几条未读);`agent_read{sid,n:0}` 不返回已完成 turn 的正文,仍可给出在飞叙述。重复读取时保存返回的 `cursor`,再传 `since:<cursor>`,只读之后完成的 turn 与当前 `in_flight`;`n:1` 读最新已完成 turn,可能重复;`turn:<id>` 是精确定位,不是去重规则。`max_chars` 由序列化后的 `turns`、`requests`、`in_flight` 共用,少量控制字段与遗漏计数在预算外。终结请求历史用 `history:true` 按需读取。这些都不是轮询:自己雇的会话以完成通知为准;永远不需要你自己去 tail 它的 `turns.jsonl`。
...
| 「它半天没动静」 | 它在**干活(working)**,不是卡住 —— Claude 子会话自己起了后台活时,一条任务要跨好几个 turn 才结束,中间一直是 `working`。去干别的,一会儿回来看结论。 |
| 「找不到项目」 | 你不在已注册项目目录里。`cd` 进去,或把项目名说出来让会话带上 `project:"<slug>"`。 |
| 「grok 用不了」 | 这台机器没装 grok CLI。`ccteam status` / capabilities 看这台机器实际有哪些 vendor。 |
| 「它安静了半小时 —— 是不是丢了?」 | 去看,别停它。`agent_read{sid,n:0}` 会答 `partial:true` 与 `in_flight{turn_id, narration, text}`:此刻正在跑的那个 turn 说到哪儿了的尾段(只有公开叙述,绝不含私有推理),外加这个 turn 正在答你的哪几条任务。读它什么都不改变、也不了结任何请求 —— turn 结束时完成通知照旧到达。`narration:"unknown"` 是「这条 harness 报不出在飞 turn 的话」,不等于沉默。 |
...
- **通知在边界批量投递,内容不合并;不丢弃、不去重、不改顺序。**显式 stop 不取消已排队的任务,ccteam 也绝不替你重发。
- **存储出错仍保留排队输入。**`pending_turns.jsonl` 有任一行不可读时,整次取出失败并报错,原文件完整保留待修复。通知预写失败时暂停投递,等下一边界或恢复重试;在退休状态持久化之前崩溃,已消费批次可能带重复标记重放,其中剩余计数保留原批次创建时的快照。
