...
/// reserved `ccteam` key (that entry is owned by
/// [`merge_project_mcp_json`]).
...
/// next start). Same merge semantics as [`merge_project_mcp_json`]: every
/// other top-level key and every other `mcpServers.*` entry is preserved,
/// and re-running with the same entry is byte-idempotent. **Config write
/// only** — ccteam never fetches, installs, or executes the server.
...
mod tests {
    ...
    #[test]
    fn named_merge_adds_third_party_and_preserves_ccteam() {
        let base = render_project_mcp_json(&bin()).unwrap();
