...
describe('import material', () => {
  ...
  function specifiersOf(file: string): string[] {
    // Type-only imports are erased at build and never become a runtime edge
    // (that is how the contract pulls upstream SlotMap merges); only value
    // imports are material.
    const text = readFileSync(file, 'utf8').replace(/import\s+type\s+[\s\S]*?from\s+'[^']+'/g, '')
  ...
  it('imports nothing from ccteam-web (structural: specifiers, not prose)', () => {
    for (const file of sourceFiles) {
      for (const specifier of [...readFileSync(file, 'utf8').matchAll(/from\s+'([^']+)'/g)].map(match => match[1]!)) {
        expect(specifier.includes('ccteam-web'), `${file} imports "${specifier}"`).toBe(false)
      }
    }
  })

  it('imports only relative modules, the client-bundle externals, and inline-safe libraries', () => {
    const allowlist = new Set([
      ...
      // Inlined into the bundle, like every DSH client package does.
      'clsx',
    ])
    for (const file of sourceFiles) {
