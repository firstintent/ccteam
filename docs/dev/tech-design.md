...
| 原则 | 对应痛点 | 落地约束 |
|---|---|---|
| **守护进程化** | 痛点 9:AI 团队需要人来主持 | gateway daemon 独立于任何 Claude Code 主对话;**pid-detach 自管**(`ccteam daemon start`,setsid,v0.9.7 起废 systemd/launchd,借鉴 codex `app-server-daemon`);SIGTERM / `ccteam daemon stop` → daemon 优雅 drain(web / IM gateway / MCP socket,每任务 ≤ 5s)。**daemon 只有一种起法 = launcher**(`ccteam start` ≡ `daemon start`;前台主体 = 隐藏 `internal daemon-run`,仅供 launcher exec —— 前台起法已删,§2.2)。**不 tick、无 orchestrator 循环**。 |
...
| **resume-by-session-id 会话** | 痛点 8/9 | session 按需 spawn(首条消息触发)+ 按持久 sid resume + **空闲释放**(主策略,`sessions.idle_release_secs` 默认 3600 = Claude prompt-cache TTL;`sessions.max_live` 是次级上限);不常驻吊着(state 落盘,重启只 reconcile 不 re-spawn),避免影子 SoT(R4) |
...
`ccteam start`(≡ `daemon start`,launcher)拉起的 daemon 本体 `internal daemon-run` 是一个常驻 gateway daemon,**不是** tick loop / orchestrator 循环(launcher 负责 setsid pid-detach + 就绪轮询 + pid 记录,见本节末「daemon 启动模型 + 安装面」与 §10 daemon 生命周期行)。它在同一个 tokio runtime 内、共享一条 shutdown 信号(Ctrl-C / SIGTERM;v0.9.7 起 trigger-file 通道已废,`ccteam daemon stop` 发 SIGTERM),启动以下任务:
...
**daemon 启动模型(v0.10.5 D7,owner 直令 2026-08-29)**:**daemon 只有一种起法 = launcher**。顶层 `ccteam start` 是 `ccteam daemon start` 的别名(与 `stop` ≡ `daemon stop` 对称):`acquire_operation_lock` + `start_managed`(setsid 脱离 + 就绪轮询 + `state/orchestrator.pid` JSON 记录,记录里含 launcher argv 供 restart 重放)→ 单行 verdict `started{pid,version,home}` | `alreadyRunning{pid,version,home}`(两者都是成功;`home` = canonical `$CCTEAM_HOME`,让调用方判断「找到的是不是我的 daemon」)。前台主体 = 隐藏 `ccteam internal daemon-run`(launcher 唯一 exec 目标;运行时 flags `--no-web`/`--no-imd`/`--web-bind`/`--dsh-web-bind`/… 全部是 launcher flags,逐字转发给它,`DaemonRunArgs` 一处定义);**MCP socket 双实例守卫(v0.9.7 F1.6)留在本体**:`mcp.sock` 已有实例应答即 bail,绝不抢 socket。前台起法删除的理由:`nohup ccteam start &` 起的实例是非托管的——`daemon stop` 拒绝、插件管不了、`daemon start` 只能报 alreadyRunning——与托管实例打架;删掉它,daemon 就只有托管一种形态。**身份面**:`GET /health`(auth 层外)= `{status,version,build?,home,pid,web_bind,dsh_web_bind?,uptime_secs}`,在双 listener 绑定**之后**构造(`:0` 报实际端口);`run/daemon-endpoint.json`(pid + 实际 `web_bind`,只在发布进程活着时可读)是**指针非身份**——身份以 `/health` 为准,launcher 记录的 argv 只说明「请求了什么」;`daemon status --json` = `/health` 字段集 + `binary`,`/health` 不可达时相应字段 `null`(不知道就说不知道,不拿请求值冒充服务值)。`ccteam update` / 无参 `daemon restart` 的重放阶梯 = 记录 argv → `/health` binds → **拒绝**(绝不回落编译期默认把 daemon 搬到别的端口)。

**安装面(v0.10.5 D4,owner 2026-08-29 改口)**:**ccteam 是一个二进制引擎、两个安装/托管面** —— CLI(`install.sh` / `make install` / `ccteam update`)与 DSH 插件 `@ccteam/ccteam-ui`(npm;插件自带引擎平台包 `@ccteam/engine-<os>-<cpu>`(`optionalDependencies` + `os/cpu` 字段、零 lifecycle 脚本,因 DSH profile 是 pnpm workspace 且 pnpm 10 默认拦 postinstall),`apply()` 探 `/health` → attach,否则按 `autoStart` 从平台包装引擎到 **install.sh 同一梯度位置**(`$CCTEAM_INSTALL_DIR` → PATH 上 `ccteam` 所在目录(解软链、排除 cargo build tree、须可写)→ `~/.local/bin`;symlink / 包管理器目录只报告不覆盖)再 `ccteam start --json`)。二者共用**同一个** `$CCTEAM_HOME` 与**同一个 daemon**:谁先起谁赢、其余 attach(按 `/health.home` 逐字节比对,不同 home = `Mismatch{home}` 只报不起第二个);插件被释放(DSH 重启 / `dsh plugin update`)**不停** daemon(disposer 只放探针);卡上「停止」= 显式 `ccteam daemon stop`(连 ccteam web / IM 一起停)。**版本锁步**(pre-1.0,D5):插件 `package.json.ccteam.engine` = 精确引擎版本 = 四平台包版本 = tag;daemon 未跑时插件把二进制对齐到钉死版本再起,daemon 在跑时**绝不**换它的二进制——`Mismatch{version}` 单向修复:引擎旧 → 卡上「更新引擎」= `ccteam update --channel npm --binary <平台包 bin>`(校验可执行 + `--version` 可解析 → 同梯度装 → 同一 drain + graceful restart + version verify 合同),插件旧 → `dsh plugin --profile web update @ccteam/ccteam-ui`。**反向不回环**:物化器(MergeOnly,operator 真 `~/.dsh`)见 profile 已由 operator 自装 `@ccteam/ccteam-ui`(判据 = **归属**而非存在:pnpm 依赖表有该包 ∨ 包目录不是 ccteam 的软链)→ 不解 tgz、不加 bundle 行、不 symlink,只写自己的 config override 行;版本 ≠ 内嵌 → `doctor` WARN + Hosts dsh 行 `plugin_version_mismatch{installed,embedded}`(同族 finding 码:`plugin_version_unknown{id}` / `plugin_missing_on_disk{id}` / `duplicate_bundle_id{id,count}` / `duplicate_patch_row{id,count}`;判据 = `ScopeOwnership` 所有权门,见 `materialize.rs` 门处 doc comment),不覆盖。凭据卫生:插件 bootstrap **只读一个文件** `secrets/web-token`(同 uid),enrollment 向 daemon 领(`POST /api/v1/enroll` `{label:"dsh-plugin:<profile>", ensure:true}`,幂等归 daemon:`ccteam_core::enroll::ensure_in` 按 (owner, scope, label) 一槽一凭据 —— 201 带 bearer = 刚铸、存进设置卡;200 只带 `bearer_prefix` = 已存在(secret 不再可读),插件比对自己持有的前缀,不符即 `rotate:true` 换新并吊销旧记录,绝不堆第二条;label 从模块路径 `profiles/<name>` 取,取不到则目录 sha 摘要 `dsh-plugin:path-<12hex>`,两个 profile 各占一槽。**机器凭据 = 无 label 的那条**(规划裁决 2026-08-29:`ensure_user_credential_in` 请求 `label: None`,写进五 vendor 全局配置、跨重启复用,不发明 "this machine" 字面;带 label 的插件 / 控制台记录与之互不遮蔽,老安装的无 label 记录按构造匹配,零迁移零回落)),不进 `process.env`、不回浏览器。平台 = linux/darwin × x64/arm64;Windows 诚实 `unsupported`。发布 = `release.yml::publish-npm`(tag → 四平台包 + 插件同版本;`NPM_TOKEN` 缺则 skip + summary;预发布 tag 走 `next` dist-tag)。代码坐标见 §10「(v0.10.5)插件自带引擎」行。
...
- **residency vs activity 是两条正交轴**(代码为准:`Gateway::session_residency` 与 `ccteam_core::stall::classify_session_activity`,各自唯一解析器):
  - **activity** = 会话在**做什么**(`working|idle|stale|stuck`),文件判据 ⋈ daemon 在飞 turn。
  - **residency** = ccteam **是否正握着**它的执行体:`resident`(live map 里有 vendor 进程/连接)/ `released`(没握着,但**下一条消息按 sid 恢复**)/ `stopped`(用户显式停,`SessionMeta.stopped_at`,任何 resume 路径会清掉)/ `detached`(体活过了 daemon,watcher 管)/ `external`(手起客户端账本节点,ccteam 从来没握过)。
  - 两条策略都只动 residency、**不动会话本身**:①**空闲释放**(主)——`run_idle_release_sweeper` 每 60s 扫一遍,超过该 harness 的 `idle_release_secs`(全局默认 3600,`idle_release_by_vendor` 按 vendor 覆盖,`0` = 永不释放)且「无在飞 turn + 无 HITL 等待 + 无跨 turn 后台任务(`RunningTask::outlives_turn`,vendor 自报)」就放掉进程;②**容量挤停**(次)——`sessions.max_live` 触顶时按 LRU 挤最久无活动的合格会话。两者共用同一个释放核心(`fence_out_session` + `release_session_shared`),故**都不动 `current_session` 焦点**(只有显式 stop 动),记同一条 `session_evicted` progress(`reason: capacity|idle`)+ `SessionLifecycle` 广播。
  - **重启只 reconcile 不 re-spawn**(`reconcile_restored_sessions`):spawn 根本不碰模型,所以「重启时把上次 live 的全部重开」买不到任何 prompt cache,只换来 N 个常驻 vendor 进程;首条消息付 1–2s 冷 resume 即可拿回同一段对话。boot 只跑 body gate(detached 认领 + post-mortem 排队)。
...
| join | satellite | `ccteam host join --daemon <url> --token <join-token>` → `POST /hosts/join`;落 `~/.ccteam/state/hosts/self.json`(host id + long-lived `agent_token`);本机 daemon(`ccteam start` 起的)30s 内自动拨出上线 |
...
| CLI | `ccteam host {join,mint-token,ls}`(serve/heartbeat 已删——卫星即 daemon 本体)|
...
| 会话状态行(per-turn,IM / web / MCP 三端同源)| 一个 struct + 一个渲染器 + 一个采集点:`crates/ccteam-harness/src/execution/turn_status.rs`(`TurnStatus{model,context,turn,cost_usd,tokens_total}` + `StatusIdentity` + 唯一文本渲染器 `render_status_line`,字面 `→ slug/sid (role) · vendor model · ctx N%⚠ · turn N · $x.xx 「title」`,缺字段整段省略);采集点唯一 = `ccteam-im/src/gateway.rs` `spawn_event_pump` 落 assistant 记录处(`thread_status()` + 会话账本累计,不扩 `CanonicalEvent`);携带面 = `TurnRecord.status`(turns.jsonl)/ `GatewayEvent.status`(web SSE `session_event_payload` + history `turn_to_event`)/ IM 回复尾行与 `/status` 头行 / `delegation.rs` **完成**通知第二行 = 指标行 `render_status_metrics`(`ctx N% · turn N · $x`,interim 通知不带)/ `session_collect|dispatch|spawn` 信封与 `session_list` 行只给数字 `context_pct`;**只在 vendor turn 边界**(一个 turn 多条 assistant 消息只有末条带,`turn` = vendor turn 序号);各前端只携带不自算 | `cargo test -p ccteam-harness turn_status`;`cargo test -p ccteam-im turn_answer_carries` |
...
| **(v0.9.15)DSH 第七 vendor(插件即集成 —— 官方 harness 无产品化自动化面 / 无全局 MCP 缝,故 ccteam 自己的 Cordis 插件坐进 DSH 树内当传输 + 工具双面孔)** | `execution/dsh_acp/`(`DshAcpAdapter`,坐共享 `execution/acp/`;`handshake.rs` 认对端 `agentInfo.name` 必须是 `ccteam-dsh*` 且广告 `agentCapabilities.loadSession`,官方 demo 名字 `deepseek-harness-acp` 显式拒;resume 阶梯 `meta.vendor_uuid` 有值→`session/load`,失败回落新 `session/new` 而非静默;**冷 resume 真做**,非豁免)· 执行/传输/凭据面已一元化(**权威 = 下方 v0.10.3 行**:connect 代替 spawn、`_meta.ccteam` per-session 凭据、身份运行时)· `dsh_acp/materialize.rs`(K25:`include_bytes!` 内嵌 tarball,sha256 缓存解压到 `$CCTEAM_HOME/runtime/dsh/<ns>/<sha>/`,幂等物化进身份 profile;当前坐标见下方 v0.10.4 行)· **插件 = `plugins/ccteam-ui/`**(独立 npm 包,不进 Rust workspace;双面孔:工具面 8 原名工具 HTTP `POST /mcp` 受管 per-sid bearer / 手起 enrollment,传输面见 v0.10.3 行;`ToolSurfaceMode::ManagedSessionBridge` 复用,文案按 vendor 分词说「plugin」不是「bridge」)· 双形态对称:方式一(ccteam web/IM 雇 DSH,受管进 live map)· 方式二(`dsh plugin add` 手起,external 节点不进 live map,完成通知经 `agent.followup` 投普通 user turn 回发起会话)· 远程 = `HostExecutionScope::LocalOnly`;价表 = `None`(真 token 上账,无 USD) | `cargo test -p ccteam-harness --test dsh_acp_test`(fake ACP 对端,锁 resume 阶梯 + `mcpServers` 零碰 + `session/new` 无键)· `-p ccteam-harness --lib execution::dsh_acp::materialize`(sha 缓存 + 物化幂等,tempdir 确定性)· `plugins/ccteam-ui && npx vitest run`(D19 bearer-scrub 断言 + 8 工具注册 + gate 逻辑);真机双模式 DoD(SIGKILL 后 `session/load` 原文召回密码;手起插件雇 claude,`parent_sid` 挂对且 external 节点确认不在 live map)留痕 `docs-local/versions/v0-9-15/w0-logs/` |
| **(v0.10.0)DSH web 一等公民(ccteam web「DSH」菜单,伴生端口反代 + 每身份隔离空间)** | **架构 = 伴生端口反代**(案 B,`/dsh/*` 路径前缀反代已由 W1 真机证伪——DSH 资产绝对路径、上游无 base-path):`ccteam-web/src/dsh_web.rs`(`DshWebSupervisor`,owner_tag→实例;operator = 真 `~/.dsh`,attach-if-detected `127.0.0.1:3080` 绝不双写;tenant = `$CCTEAM_HOME/runtime/dsh/web/<user>/` 合并式物化,只保证 ccteam 三件 bundle 在场、用户自装插件层原样保留、凭据出厂为空走 DSH 原生 Settings→Models 自助)· 代理 = Host/Origin 重写 + 剥 Cookie/`sec-fetch-*`/hop-by-hop + `Accept-Encoding` 剥除(保证响应体恒 identity 编码)+ WS 双向泵(`hosts.rs` exec 泵同构)+ **HTML 响应即时缓冲注入 secure-context polyfill**(`crypto.randomUUID` 在非 loopback 明文源不可用,`inject_polyfill` 补 `crypto.getRandomValues` 实现的真 RFC4122 v4,原生已有则整段 no-op)· REST `/api/v1/dsh/{status,start,stop}` = `routes/dsh.rs`(`state:stopped|starting|running|attached|disabled`,`native_url` 仅 operator + running/attached 下发)· 租户 profile patch 生成 = `execution/dsh_acp/materialize.rs::profile_patch_yaml`(**override 非 insert**——ccteam 的 bundle 已在 `dsh.profile.bundles` 里、其自带 patch 层已插入同 id 行(现行 = `ccteam-ui`),二次 insert 令 Cordis 以 `duplicate loader entry id` 拒绝整个 boot;`config` 扁平不嵌套命名空间,行内容才到得了 `apply(ctx, config)`)· SPA `DshView.tsx` + `Sidebar` 顶级菜单 + operator 专属「打开原生窗口」(`native_url` 存在且浏览器在 loopback 才展示)。**两个真机死锁/启动 bug 均由此表现修复**:①`start_for` 已在跑快捷路径持锁跨 `.await` 自调 `status_for` 二次抢同一把 `tokio::sync::Mutex`——不可重入,永久卡死后续一切碰该锁的调用(含 `/status`);②租户 patch 的 `insert` 双写 + 嵌套 config,回归测试断言 patch 结构(零 `insert:` 键 + 扁平 config)而非子串 | `cargo test -p ccteam-web --test dsh_web_test`(companion 无身份 401 · disabled 形状 · **`repeated_start_on_an_already_attached_instance_never_deadlocks`**——两次 start + 一次 status 全套 5s timeout guard,回归前 `Elapsed` 失败)· `-p ccteam-web --lib dsh_web`(polyfill 注入顺序/无 head 时惰性/仅 text/html 缓冲 7 例)· `-p ccteam-harness --lib execution::dsh_acp::materialize`(`merge_preserves_self_installed_profile_layer` 结构断言 patch 形状,非子串)· vitest `DshView.test.tsx`/`Sidebar.test.tsx`;真机三 bug 逐条复现(裸 socket 直连源 + 独立 axum/reqwest 复现代码排除通用陷阱、锁定 ccteam 自身锁逻辑)+ 修复后 operator/tenant 双身份端到端验证,留痕本对话 |
| **(v0.10.3)DSH 会话一元化(一个身份一个 DSH 运行时,ccteam 是它的第二个 client)** | 雇佣 = **连接非进程**:`acp/transport.rs::connect_unix`(坐既有流泛型泵,child=None,断连绝不杀运行时)→ 插件在身份运行时(`dsh web`,`execution/dsh_runtime.rs::DshRuntimeManager` 管,cli 装配单 Arc 同时交 adapter/gateway 与 web 反代)内按 profile patch 的扁平 config `transportSocket` 自绑 unix socket(`$CCTEAM_HOME/runtime/dsh/acp/<segment>.sock`,注意 `sun_path` 108B)serve ACP,多连接每连接一对端 · 身份家/socket 单一来源 = `spawn_spec.rs::{identity_dsh_home, identity_socket_path}`(`user:<id>`→租户 web 家,其余 owner tag 全部塌缩 operator 真 `~/.dsh` 单实例键)· per-session 凭据 = `session/new|load` 的 `_meta.ccteam{sid,bearer,mcpUrl,approvalMode}`(零 env,取代 D19 自净;工具面按 `exec.agent.id` 路由 per-sid bearer,查无回落 enrollment)· **turn 归属过滤**(账本诚实):插件凭自己 `UserMessage.id` 绑 turn 号,只转发归属 turn 的 assistant/tool 事件,完成 = 归属 `turn/end`;人从 DSH 侧输入的 turn = vendor 原生,不进 ccteam transcript/账本,审批对非归属 turn 放行 `next()` · `session/new` 先 `workspaceRegistry.create(cwd)` 串行链 + `attachSession`(侧栏实时 = DSH 自身 `session/created`→mux 推送)· `session/load` reuse-live-first(resume 对 live 会话 vendor 拒),断连不 dispose;stop/挤停 = `session/cancel`(request,非 notification)+ 断连 · 版本门 = `initialize` 的 `agentInfo.version >= 0.10.3`(argv 探测已删)· 注册面(门①)= `materialize.rs::register_ccteam_plugins_into_profile`(MergeOnly:package.json bundles + node_modules 物化 + `cordis.patch.yml` 自己的 override 行,恒 override 非 insert)—— 受 `POST /hosts/local/register-mcp?vendor=dsh`(Hosts 页 DSH 行按钮,admin)与 operator spawn 分支调用;attach 到手起实例缺插件 = 可读 remedy,ccteam 绝不重启他人进程 · per-sid `DSH_HOME`/env 传输开关/凭据镜像/managed profile 全部退役(历史 per-sid 会话 vendor 侧 resume 不迁移,legacy 清扫收残留) | `cargo test -p ccteam-harness --test dsh_acp_test`(fake = unix-socket ACP server,`CCTEAM_DSH_SOCKET` 测试注入;`_meta` 携带/版本门/同身份单运行时/cancel→断连 fake 存活)· `-p ccteam-web --test dsh_register_test`(注册端点 merge-only + 幂等)· `plugins/ccteam-ui && npx vitest run`(多连接隔离/零 env/turn 归属人机混流/审批放行/cancel 双臂)· vitest HostsView(DSH 行注册 CTA admin/local 门)|
| **(v0.10.4)ccteam-ui —— 一个插件承载三面(工具 + 传输 + UI)** | **定位** = 在 DSH 的 UI 里用多种 harness,未来可能整体取代 ccteam web,故一切按 DSH 客户端插件机制构建、零 ccteam-web 移植。**唯一插件 = `plugins/ccteam-ui/`**(v0.10.4 前的第二个插件已整目录删除,零 alias 零兼容分支、零迁移步骤——退役包由 `@ccteam/` scope 的通用清理路径自动清掉):host 半 = `src/index.ts`(**每面各自 `ctx.inject` 自己的服务**——`webServer` 只在 web-app bundle 里,并集会把整插件连工具面一起在非 web profile 上锁死,与 `transport.ts` 记的 `workspaceRegistry` 同一个坑)+ `src/{tools,transport,credentials}.ts`(8 原名工具 / 自绑 stdio serve ACP + `session/load` / per-session `_meta.ccteam` 凭据)+ `src/{bff,sse}.ts`(`/ccteam/api` BFF);client 半 = `src/client/index.tsx` 经 `ctx.slots.inject` 注册 `sidebar.footer.action`/`shell.overlay`/`settings.plugin.item` 三座位,业务状态走 inject face `hooks` 舱由框架合成 selector hook,样式 = lightningcss CSS Modules `[hash]_[local]`(`build/css-plugins.ts`,复刻 DSH 预设;手搓 hash 以数字开头曾让整张样式表失效),上游类型 type-only 引自 npm `next` 0.1.1-rc.2。**设置 = 一个扁平 namespace `ccteam-ui`**(`src/settings.ts`:daemonUrl 只填一次 + `enrollment`(进程身份,工具面)+ `restToken`(人身份,工作台)+ `defaultProject`),它同时就是 profile patch 行 `config` 的形状,故键必须扁平。**包名规则**:`@ccteam/<product>-<name>` —— DSH 插件列表剥 `dsh-` 前缀不剥 `ccteam-`,全称显示。**物化通用解**:`materialize.rs` `CCTEAM_PLUGINS` 表(bundle/package/row_id/cache_ns/required_config_key/tgz,现 1 行)+ 一个 `DshPluginConfig` + 物化器接管 `@ccteam/` scope(表外 bundle/patch 行/scope 链接一律清除 —— 退役包由这条通用解自动清掉,不写迁移步骤;scope 外零改动)。ACP 对端名仍是 `ccteam-dsh-client`(wire 契约,与 `handshake.rs` 的前缀门同步) | `plugins/ccteam-ui && npm run -s typecheck && npx vitest run`(162 例:三面 + 设置 schema 结构断言 + 客户端座位/材料门 + PACKAGE_VERSION 与 package.json 一致)· `-p ccteam-harness --lib execution::dsh_acp::materialize`(单插件表 / 一行 patch / 陈旧 scope 清理)· `--test dsh_acp_test` · `-p ccteam-web --test dsh_register_test` |
| **(v0.10.5)插件自带引擎 + 单 daemon 共存(一个二进制引擎、两个安装/托管面;§2.2「daemon 启动模型 + 安装面」)** | `/health` 身份 = `crates/ccteam-web/src/routes/health.rs`(`HealthIdentity{home,build,web_bind,dsh_web_bind,started_at}`,`bound` 在双 listener 绑定后构造,auth 层外)+ `lib.rs`(`ServeOpts{build,publish_endpoint}`,只有 daemon 本体发布 endpoint,`internal web` 不发)· endpoint 指针 = `crates/ccteam-core/src/daemon.rs`(`run/daemon-endpoint.json` = `DaemonEndpoint{pid,web_bind}`,`read_endpoint` 只认活 pid;**指针非身份**)+ `PidRecord.args`(launcher argv,restart 重放用)· launcher / verdict / status / replay = `crates/ccteam-cli/src/daemon_cli.rs`(`run_daemon_start` 两 verdict 均带 `home`、`live_health` 只认同 home 的应答、`run_daemon_status` 输出 `/health` 字段集 + `binary`、`plan_replay` 三阶:记录 argv → `/health` binds → `ReplayUnavailable` 拒绝)+ `main.rs`(`Command::Start(LaunchArgs)`/`DaemonCommand::Start` 共用 `DaemonRunArgs` 转发;`InternalCommand::DaemonRun` = 本体 + F1.6 守卫)· 安装梯度 + npm channel = `crates/ccteam-cli/src/update.rs`(`resolve_install_dir_with` 镜像 `install.sh` 三级、`classify_destination` symlink/包管理器拒写、`run_local_binary_update` = 校验 → 装 → marker → 同一重启合同 `run_restart_contract`;drift 守卫 `install_sh_ladder_rungs_match_the_rust_copy` 读 `install.sh` 源)· 物化器 skip-if-installed = `execution/dsh_acp/materialize.rs`(`operator_install_of` 按**归属**判 → `PluginPlan::KeepOperatorInstall` 只写 config 行;`ccteam_plugin_findings` → `ccteam-cli/src/doctor.rs::check_dsh_plugin_health` WARN + `ccteam-web/src/dsh_web.rs::operator_dsh_plugin_findings` → `routes/hosts.rs` dsh 行 `plugin_version_mismatch`)· 插件引擎托管 = `plugins/ccteam-ui/src/host/engine/{locate,install,supervisor,bootstrap}.ts`(`locateEngine` 镜像 Rust 的 home/梯度/版本解析;`installEngine` copy→chmod→verify→rename;`EngineSupervisor` 状态机 `unsupported|missing|installing|stopped|starting|running|attached|mismatch{home|version}` + `ensure()`/`start|stop|restart|update|log` + `dispose` 只放探针;`createTokenBootstrap` 只读 `secrets/web-token`、`createEnrollmentBootstrap` = ensure(label `dsh-plugin:<profile>`)+ `bearer_prefix` 比对 + `rotate`,幂等归 daemon)+ `src/shared/contract.ts`(`engine.*` 六方法 + `EngineStatus`)+ `src/settings.ts`(`autoStart`/`enginePath`)+ `package.json`(`ccteam.engine` 钉版 + 四 `optionalDependencies`)· 平台包 = `plugins/engine-packages/`(四 `package.json` 模板 `os/cpu` + `pack-engine.sh`,bin 755 断言)· npm 发布 = `.github/workflows/release.yml::publish-npm`(tag → 四平台包 + 插件同版本,`ccteam.engine` = tag;`NPM_TOKEN` 缺 → skip + summary;预发布 tag → `next`;已发布版本幂等跳过) | `cargo test -p ccteam-cli --bins`(daemon_cli / update)+ `--test daemon_identity_test --test update_npm_channel_test --test cli_surface_test`(`start --help` 无前台措辞);`-p ccteam-web --lib routes::health`;`-p ccteam-harness --test dsh_acp_test`(operator-installed 族)+ `-p ccteam-web --test dsh_register_test`;`plugins/ccteam-ui && npx vitest run`(`host-engine*.test.ts`,含梯度 drift 守卫读 `install.sh`);真机沙箱 DoD(干净 home 一条命令 / 先 CLI 后 DSH attach 同 pid / 先 DSH 后 `ccteam start` = `alreadyRunning` 同 pid / DSH 重启 daemon 不受影响)留痕 `/tmp/ccteam-ui-dod/plug3/out/` |
