...
function fakeSettings(stored: Partial<CcteamTeamSettings> = {}) {
...
describe('settings registration', () => {
  it('registers one namespace whose schema carries the documented fields', () => {
    ...
    registerCcteamTeamSettings({ settings: service })

    expect(registrations).toHaveLength(1)
    const [registration] = registrations
    expect(registration!.ns).toBe(SETTINGS_NAMESPACE)
    expect(registration!.ns).toBe('ccteam-team')
  ...
  it('still resolves values when the settings service is absent', () => {
    const scope = registerCcteamTeamSettings({}, { daemonUrl: 'http://elsewhere:1234' })
  ...
  it('defaults the daemon URL when neither config nor settings supply one', () => {
    expect(registerCcteamTeamSettings({}).get().daemonUrl).toBe(DEFAULT_DAEMON_URL)
  })
})
...
describe('config-over-settings precedence', () => {
  ...
  async function probe(
    config: Partial<CcteamTeamSettings>,
    stored: Partial<CcteamTeamSettings>,
    payload: unknown = {},
    method = 'status',
  ): Promise<FakeFetch> {
