...
| 当前在做 | **v0.9.0-0.9.2 = Agent2Agent 底座已落 dev(未 tag、未部署)**。铁律:**只做单 harness 做不到的**(跨 vendor 身份/路由/账本/观测 + 跨机执行),**永不做厂商能力**。v0.9.0 W1-W4(principal 调用面 / 委派语义+引擎中立化 / 反向连接跨机 / 团队可视化)+ v0.9.1(MCP 单前缀 + 主会话 fallback)+ **v0.9.2(owner 六点:项目↔主机绑定(host 归 project、spawn 去 host 参、`project_init` op、import)· live 容量 50 LRU 挤停 · 团队拓扑树替换泳道 SVG · 会话 vendor chips · A2A 返回限幅防父会话膨胀 · 新建项目自由选主机)** 详见 §〇/§三/§四 + `docs-local/versions/v0-9-{0,1,2}/`。**剩 W5**:hub 示例配方 + 三场景真机 smoke + README/usage 重写;backlog P1 = codex turn 粒度折叠(叙述消息≠独立 turn)+ collect 游标去重。**tag/部署 HELD。逐版改动史 = `git log` + `docs-local/versions/`(gitignored);协议一律以代码为准。** |
...
| **session 调度门 = daemon 校验 per-session principal `(sid,secret)`(best-effort,非硬边界;v0.9.0 泛化,去 cto-only)** | 5 个 `session_*` 工具(spawn/dispatch/collect/list/stop)由 daemon 按 **principal** 校验(**任意有效 session,不再限 cto role**):spawn 时 mint per-session secret 注入子会话 MCP env(`CCTEAM_CHAT_SECRET`)+ per-session `mcp.json`/ACP `mcpServers` bearer,`verify_session_principal(sid,secret)` 常数时比对 → 返回 `CallerCtx{sid,slug,role}`,`_caller_slug` **服务端覆写**(不信 caller 自报,只能操作自己 project);HTTP `/mcp` sid-bearer 同路。role 降为审计标签、不参与授权;`dispatch`/`stop` 是显式调度(非主动 kill),stop 限后代 + depth/children/delegated/cycle/预算护栏。**主会话 fallback(v0.9.1)**:非 ccteam 拉起的本机日常会话(无 principal env)由 stdio forwarder 读 `~/.ccteam/secrets/web-token`(0600,同 uid 证明)转发 `_caller_admin_token`,daemon 常数时比对后按 **Admin 语义**服务(root spawn 无 parent、project = 显式参数或 cwd 解析;与 HTTP admin bearer 同信任级)——**仅本地 socket**,无 token 仍 fail-closed;HTTP 路 `inject_session_caller` 剥该参数。**诚实范围**:单 OS-uid 全信任模型下 agent 间**无硬边界**(同 uid 可读他进程 env/文件/ptrace → 拿 secret),secret 只**抬门槛**(defense-in-depth),**不 close**;真隔离 = per-agent OS user / sandbox(deferred) |
...
| **MCP** | `ccteam` **8 工具**,0 STUB(v0.9-T1 cull 15→8,删 advise 2 / admin_change_persona+add_tool / chat bot 3,`admin_ls`→`status`;`STUB_TOOLS` const + `ccteam doctor --verify-mcp` 自检,drift exit 1):`status` · `chat_send_file` · `screenshot` · **session 5**(**A2A 委派,任意 session 可调,非 cto-only**:`session_spawn`(vendor/model/effort/protocol/title;**无 host —— 继承 project 绑定,传入即错**)/`dispatch`(wait/notify/idempotency_key)/`collect`(`max_chars` 默认 10k,字段级 head70/tail30 截断 + 账本指针)/`list`(树)/`stop`,daemon 校验 per-session principal `(sid,secret)` + project 维度 + 护栏(depth 2 / children 10 / delegated 50);wait 内联答案 10k、完成通知嵌入 4k 限幅防父会话膨胀);wire 名裸(`session_spawn` 等,客户端按 server key 加命名空间 → 模型见 `mcp__ccteam__session_spawn`,v0.9.1 修双前缀);**主会话可调**(无 principal env 时 forwarder 以 web-token 走 Admin 语义,cwd 解析 project);入口 = stdio `mcp-serve`(stdout 纯 JSON-RPC、tracing→stderr)+ `POST /mcp` streamable HTTP(admin bearer 或 sid-bearer `ccteam-sid:<sid>:<secret>`)|
...
| **Plugins** | **ccteam 是纯 CLI、不是 vendor 插件**:MCP server 由 `ccteam config` 注册 —— 同时写 Claude `~/.claude.json` + Codex `~/.codex/config.toml`(`mcp_serve::install_mcp` / `install_codex_mcp`,canonical `internal mcp-serve`),per-project `.mcp.json` 由 web「MCP servers」页按需写(`merge_project_mcp_json`;init 不写);repo **不**带 `.claude-plugin`/`.codex-plugin`/`marketplace.json`/根 `.mcp.json`。提示词类插件(skill/role/workflow)住 ccteam-hub |
