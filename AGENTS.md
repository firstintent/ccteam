...
| 项 | 值 |
|---|---|
| Workspace version | `0.10.5`(**已发布** 2026-08-29:owner「ship」→ PR #188 merge commit `38abbffe` → tag `v0.10.5` → release run 33260307619 四平台 + npm 全绿;合并 v0.10.4 周期(ccteam-ui 整页工作台 + LIFE-1)与 v0.10.5 波(插件自带引擎)一次发版,循 v0.10.3 先例取最高周期号。上一版 = v0.10.3(已发布 2026-08-19)) |
| 本版 headline | **插件自带引擎 + 单 daemon 共存版**:① **DSH 插件合一** —— `@ccteam/ccteam-client` 并入 `@ccteam/ccteam-ui`(`plugins/ccteam-ui/`),一个 npm 包三面(工具面 / 传输面 / UI 工作台),pack 字节可复现;② **`dsh plugin --profile <name> add @ccteam/ccteam-ui` = 完整 ccteam** —— 插件自带引擎平台包 `@ccteam/engine-<os>-<cpu>`(optionalDependencies,零 lifecycle 脚本),`apply()` 装引擎(与 `install.sh` 同一梯度)→ 探 `/health` attach、否则 `ccteam start`(默认自启);设置卡「引擎」段(状态机 / 启停重启更新 / 自启 / 日志尾)+ 工作台首启三态 + 版本锁步横幅(插件 `ccteam.engine` == 引擎版本,单向修复);③ **单 daemon 共存不变量** —— 原生 DSH / ccteam web / CLI / systemd 同 `$CCTEAM_HOME` 同一个 daemon,谁先起谁赢其余 attach,插件释放不停 daemon;`/health` 身份字段 + `run/daemon-endpoint.json` pid 门指针;物化器**所有权门**(用户自装 `@ccteam/*` 永不重复物化/剪枝,重复 id / 悬空行只报告,注册幂等到零写);④ **D7:前台 `ccteam start` 永久删除** —— `start` ≡ `daemon start`(launcher),前台主体 = 隐藏 `internal daemon-run`;`ccteam update --channel npm --binary`;回放缺记录即拒绝不猜;⑤ **enrollment 幂等 ensure**(`POST /api/v1/enroll {label, ensure}`;机器凭据 = 无 label 槽,user 作用域无 label 一律 400);⑥ **会话生命周期 = activity × residency**(LIFE-1:idle-release 主机制、`max_live` 次级、重启懒重建、released 会话全面可见并按 sid 复活)+ ccteam-ui 整页工作台(团队树 | 原生级会话 | 详情;行菜单 / 中途切模型 / 状态真相);⑦ 发布面:tag → 四平台二进制 + npm(`@ccteam/engine-*` + 插件同版本,`NPM_TOKEN`);测试卫生:全 workspace `#[cfg(test)]` 禁 env 写守卫 + 17 处注入根。细节与教训见 `.loop/history.md` / `docs-local/versions/v0-10-5/`。 |
