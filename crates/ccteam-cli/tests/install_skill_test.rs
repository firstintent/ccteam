//! `ccteam doctor --install-skill [NAME]` test harness.
//!
//! The shipped skill set is **two** entries:
//!   `ccteam-control` / `ccteam-creator`
//! The flag is `Option<String>` so users can install one at a time
//! (`--install-skill ccteam-creator`) or all (`--install-skill all` /
//! plain `--install-skill`).
//!
...
#[test]
fn install_skill_default_lays_down_all_shipped_skills() {
    // `--install-skill` with no value (or `all`) writes every shipped
    // skill into `~/.claude/skills/<name>/SKILL.md`. The set is 2
    // entries:
    //   ccteam-control / ccteam-creator
    //
    ...
    for skill in ["ccteam-control", "ccteam-creator"] {
...
fn install_skill_all_explicit_value_matches_default() {
    ...
    // Same 2-skill default set must land.
    let tmp = TempDir::new().unwrap();
...
fn install_skill_single_name_writes_just_that_skill() {
    // V0.5.0 F93a: `--install-skill ccteam-creator` is single-skill
    // mode — only the named skill lands; the other shipped skills are
    // NOT written.
    let tmp = TempDir::new().unwrap();
    let out = run_ccteam(&["doctor", "--install-skill", "ccteam-creator"], &tmp);
    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        out.status.success(),
        "doctor --install-skill ccteam-creator should succeed; stdout={stdout}; stderr={stderr}",
    );
    let claude = tmp.path().join("isolated").join(".claude");
    assert!(
        claude
            ...
            .join("SKILL.md")
            .is_file(),
        "ccteam-creator SKILL.md should have been written; stdout={stdout}",
    );
    // Other shipped skills should NOT be present in single-skill mode.
    let other = "ccteam-control";
    assert!(
...
fn install_skill_is_idempotent_when_rerun() {
    // Verifies the underlying `install_skill_body_into` is re-entrant
    // (file already present → no-op without `--force`) through the full
    // CLI path so we catch e.g. a dispatch regression.
    let tmp = TempDir::new().unwrap();
    let first = run_ccteam(&["doctor", "--install-skill"], &tmp);
    assert!(first.status.success());
    let claude = tmp.path().join("isolated").join(".claude");
    let probe_skill = claude
        .join("skills")
        .join("ccteam-creator")
        .join("SKILL.md");
    let mtime_first = std::fs::metadata(&probe_skill).unwrap().modified().unwrap();
    ...
    let mtime_second = std::fs::metadata(&probe_skill).unwrap().modified().unwrap();
    assert_eq!(
        mtime_first, mtime_second,
        "ccteam-creator SKILL.md mtime changed on idempotent re-run",
    );
}

#[test]
fn install_skill_unknown_name_errors_with_friendly_message() {
    // A bogus skill name (e.g. typo) should bail with a helpful message
    // listing the canonical names. Don't bury the user in a stack
    // trace.
    let tmp = TempDir::new().unwrap();
...
fn install_skill_default_scope_is_user_level_claude_dir() {
    ...
    assert!(
        claude
            ...
            .join("SKILL.md")
            .is_file(),
        "ccteam-creator SKILL.md should land under the redirect target",
    );
