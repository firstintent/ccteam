...
| **F93** | workflow.yaml `mode: agent-team` schema + `__lead` role 工厂 + user-in-control spawn | `ccteam init --mode agent-team` 生成 `__lead.md` + workflow.yaml;`ccteam start` 走 `[Y/n/attach]` confirm prompt(不 silent spawn);`ccteam attach <slug>` 一行 attach lead 看交互;lead 默认 plan-first protocol(先报方案 → user 回复 `go` → 再 spawn teammate);支持 Anthropic 两类 teammate(definition-backed / ad-hoc);workflow.yaml `suggested_teammates[]` + `auto_spawn_teammates: false` 默认 |
