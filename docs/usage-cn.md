...
- **两种作用域**:每次 `ccteam daemon start` 会把一枚**用户域**凭据写进本机各 vendor 配置,所以手起的 Claude/Codex/Grok/OpenCode/Kimi 直接就有工具;DSH 在 `@ccteam/ccteam-ui` 插件连上粘贴的 enrollment 凭据后走同一套身份模型。用户域凭据不钉项目,故首个 `session_*` 调用必须显式传 `project`,且只接受本人可见的项目。**项目域**凭据 = 控制台复制按钮发的那枚(设置 → 接入,或 `POST /api/v1/projects/{slug}/enroll`):钉死一个 workspace,贴到别的机器也只够得着它,事后可列出、可吊销;secret 只在签发那一刻显示一次。
