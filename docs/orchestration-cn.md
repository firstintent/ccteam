...
**给忙着的同事再派一件事是常态,而且它会告诉你。** 每次 `agent` 调用都是一条有自己 id 的请求:回包直接说它是 `started`(开了新 turn)、`injected`(汇入正在跑的那个 turn)还是 `queued`(排队,并给出从 1 起算的位置),`agent_read{sid}` 则列出这个会话还欠谁什么。完成回报只答它对应的那一条请求。所以一个看着没动静的子会话是在告诉你**为什么**:它在干活,或者你的任务排在第三 —— 而不是两种情况都回同一个干巴巴的 pending。
