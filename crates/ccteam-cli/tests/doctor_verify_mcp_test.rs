...
//!
//! Tests cover:
//!   - active tool count matches the shipped surface (6)
...
#[test]
fn active_count_is_6_and_stub_count_is_0_on_clean_tree() {
    // status 1 + beacon alias 1 + chat 1 + session 3 = 6. STUB_TOOLS empty.
    ...
    assert_eq!(v["total_tools"], Value::Number(6.into()), "{v}");
    assert_eq!(v["active_count"], Value::Number(6.into()), "{v}");
...
fn json_output_schema_includes_per_group_and_tool_list() {
    ...
    // The pre-merge session names have no alias.
    for gone in [
        "session_spawn",
        "session_dispatch",
        "session_collect",
        ...
        assert!(!names.contains(&gone), "retired tool present: {gone}");
    }
...
fn human_readable_output_contains_verdict_pass_and_breakdown() {
    ...
    assert!(stdout.contains("total tools:    6"), "got: {stdout}");
    assert!(stdout.contains("active:         6"), "got: {stdout}");
...
fn human_mode_lists_every_shipped_group_with_active_count() {
    ...
    let (stdout, _stderr, code) = run_doctor_verify_mcp(&[]);
    assert_eq!(code, 0);
    for (group, active) in [("admin:", 2), ("chat:", 1), ("session:", 3)] {
