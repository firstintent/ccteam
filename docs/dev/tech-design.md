...
| **(v0.9.11)团队页驾驶舱重设计**(owner 直驱 PRD,两 tab 拓扑\|分工) | 拓扑独占(roster/timeline tab 删)+ 会话真链接(`Link` → `/chat/s/:sid`,右键新 tab 对照父子)+ per-vendor KPI chips(点击过滤)+ 委派 ticker + 多 host 徽章 + live 节点 model join(共享 helper `sessions_api::resolved_thread_status`,idle 保持 None)= `ccteam-web/src/routes/{agents.rs,sessions_api.rs}` + SPA `web/src/pages/AgentsView.tsx` · 分工 charter tab = `web/src/pages/CharterPanel.tsx` + `lib/{routingApi,charterState}.ts` + REST routing 面(见 §3 资源表 routing 行)· 编队起手 = `web/src/lib/playbooks.ts`(6 编队定义唯一家,Home launcher 与团队页双消费,router-state 一次性起手)+ `pages/HomeView.tsx` | `cargo test -p ccteam-web --test routing_test --test agents_test --test sessions_api_test`;vitest(`AgentsView`/`CharterPanel`/`playbooks`/`charterState`/`routingApi`) |
