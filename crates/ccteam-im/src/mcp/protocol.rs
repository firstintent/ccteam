...
`project`, or — when exactly one project is registered — that project). A plain admin-fallback caller is not itself \
a managed session, so it has no completion-notification return transport: async responses report \
`notify_deliverable:false`; poll `session_collect` (or use `wait_seconds`) instead.\n\n\
...
pub fn session_tool_definitions() -> Vec<Value> {
    vec![
        json!({
            ...
            "inputSchema": json!({
                ...
                "properties": {
                    ...
                    "wait_seconds": { "type": "integer", "description": "With `task`: request 0–600 seconds (default 0 = async); effective inline wait is capped at 240s. Use inline wait for health probes/short tasks; keep long/repo tasks async (managed parents get a notification; admin fallback polls collect). Pending/timeout never cancels the child." },
                    "notify": { "type": ["string", "boolean"], "description": "With `task`: for a managed parent, `final` (default) wakes it ONCE when the child's vendor turn ends; `all` wakes it on every assistant message (debug firehose); `off` = ledger-only. A plain admin-fallback caller has no notification return transport: the response reports `notify_deliverable:false`; poll session_collect. Booleans still parse: true→final, false→off." },
        ...
        json!({
            ...
            "description": "Dispatch a task to a session by `sid` (from session_spawn / session_list); the target must run in YOUR OWN project. `task` is forwarded VERBATIM as a user turn (NO system-prompt injection). Async managed-parent calls get ONE completion notification at the vendor turn boundary; a plain admin-fallback caller has no return transport, gets `notify_deliverable:false`, and must poll `session_collect` (or use `wait_seconds`). Inline completion returns `{status:\"completed\"|\"failed\", result_text, error_kind?, error?, elapsed_seconds, cost_usd?, tokens_total?}`; timeout returns `{status:\"pending\"}` and never cancels the child. Instruct children to answer tersely with a structured summary and no code or diff dumps, because answers beyond the return cap are truncated. Dispatch to yourself or an ancestor is rejected (cycle). Explicit dispatch, never a proactive kill.",
            "inputSchema": json!({
                "type": "object",
                "properties": {
                    ...
                    "wait_seconds": { "type": "integer", "description": "Request 0–600 seconds (default 0 = async); effective inline wait is capped at 240s. Use inline wait for health probes/short tasks; keep long/repo tasks async (managed parents get a notification; admin fallback polls collect). Pending/timeout never cancels the child." },
