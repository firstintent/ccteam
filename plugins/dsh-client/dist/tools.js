...
export class CcteamMcpClient {
    ...
    assertOpen() {
        if (this.closed)
            throw new Error('ccteam MCP client is disposed');
    }
}
export const CCTEAM_TOOL_DEFINITIONS = [
    {
        name: 'status',
...
export function registerCcteamTools(ctx, client, notifier) {
...
function toDshTool(definition, client, notifier) {
    return {
...
export class CcteamCompletionNotifier {
    client;
    pollIntervalMs;
    maxPolls;
    sleep;
    ...
    maybeNotify(toolName, args, result, exec) {
        if (this.closed)
            return;
        if (toolName !== 'session_spawn' && toolName !== 'session_dispatch')
            return;
        if (toolName === 'session_spawn' && (!isRecord(args) || typeof args.task !== 'string' || args.task.trim() === ''))
            return;
        const origin = exec.agent;
        if (!isAgentWithSession(origin))
            return;
    ...
    async pollAndFollowup(origin, sid, initial) {
...
function renderDelegationSummary(sid, value) {
...
function jsonRpcErrorMessage(payload) {
    if (!isRecord(payload))
        return undefined;
