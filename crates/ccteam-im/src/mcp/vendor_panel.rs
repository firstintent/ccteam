...
fn render_recipes(
    ...
    for row in rows.iter().filter(|r| r.installed) {
        let recipe = match row.vendor.as_str() {
            ...
            "dsh" => {
                "session_spawn{vendor:\"dsh\", task:\"…\"} — DeepSeek Harness via managed spawn (automatic ccteam plugin; no user action needed). External DSH plugin: dsh plugin --profile web add @ccteam/ccteam-client; has cold resume"
            }
            _ => continue,
        };
...
mod tests {
    ...
    fn recipes_render_installed_vendors_only() {
        ...
        assert!(
            out.contains("dsh plugin --profile web add @ccteam/ccteam-client"),
            "{out}"
        );
