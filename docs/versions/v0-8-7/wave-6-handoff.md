...
- **CLAUDE.md(149 行)**:§〇 版本戳 + per-session-web/OpenAPI 已落地;§一 version→0.8.7、baseline→1942/0(注 ccteam-web 230+ / 5 ws_* + vitest 108)、「当前在做」改 v0.8.7 一段式总结、MCP「12→17」+ session_ 前缀 + /api/docs + /chat/s/:sid;§三 加 2 红线行(HITL 边界 = PermissionRequest hook、hitl spawn `--permission-mode default` 非 skip、deny 不杀 turn;cto dispatch = daemon role==cto 硬门、session_* only、dispatch/stop 显式非主动 kill)+ 校正退役-HITL 注(per-session HITL 已落 vs orchestration 级 deferred);§四 MCP 17 + session 5 组 + `ccteam role search/add/list`。
