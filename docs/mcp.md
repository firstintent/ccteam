...
- `wait` — seconds to block inline, 0–240 (default 0 = async). A timeout answers `status:"pending"` and **never cancels the child**.
...
| `notify` | Excerpt | Use |
|---|---|---|
| `brief` (default) | 500 chars, head/tail, + the exact `agent_read{sid,n:1,max_chars}` call for the whole answer | the default: verdict + coordinates; the parent is the scarcest context in a team, and the full text is one precise call away |
