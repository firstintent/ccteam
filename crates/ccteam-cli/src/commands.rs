        ...
        HarnessKind::Claude => {
            ...
            let handle = adapter
                .spawn_session(SpawnOpts {
                    ...
                    // V0.4.0 F61: flex multi-session add defaults to
                    // the `main` agent role. Real workflow-driven
                    // spawns (F66) pass the explicit role from
                    // workflow.yaml::agents.<role>.
                    role: "main".to_string(),
                    slug: slug.to_string(),
                    sid: sid.clone(),
...
pub fn run_session_rm(slug: &str, sid: &str) -> Result<()> {
    ...
    let handle = SessionHandle {
        // V0.4.0 F61: Legacy state.json may not have job_id (pre-F61
        // session add). Fall back to empty string — shutdown_session
        // tolerates a missing state.json gracefully (no-op).
