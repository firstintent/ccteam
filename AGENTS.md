...
| 项 | 值 |
|---|---|
| Workspace version | `0.9.15`(**dev 完成、待发布**:owner 2026-08-15 直驱「规划把控架构 + 派 codex 子会话开发 + 推 dev + 开版本 PR」;dev→main **PR #180** 已开,待 owner 合并;merge/tag/`npm publish`(D18)仍需 owner 显式发话。上一版 = v0.9.14(已发布 2026-08-12,main `fb2a9540`)) |
| 本版 headline | **DSH(DeepSeek Harness)第七 vendor —— 插件即集成**(本版主线):DSH 无产品化自动化子命令、无全局 MCP 配置缝、官方 ACP 是刻意残缺的 demo(无 resume、无 tool/thought/usage 上线),但把插件系统(Cordis)当第一公民公共 API。ccteam 顺着这个架构做集成而非趴在残缺 demo 上:**一个 npm 包 `@ccteam/dsh-client`**(仓内 `plugins/dsh-client/`)双面孔——工具面(受管 + 手起,8 原名 MCP 工具)+ 传输面(仅受管,env 双开关,插件自绑 stdio serve ACP,含 `session/load` 架在 `ctx.agents.resume` 上、富事件、审批、取消)。Rust 侧 `execution/dsh_acp/` 坐共享 `execution/acp/`(kimi/grok 同门形状 + resume 阶梯),零新协议机器。**双形态并列一等**(owner 钦点):方式一 = ccteam 集成 DSH(ccteam web/IM 入口,受管 vendor,进 live map);方式二 = ccteam 作为 DSH 插件(DSH web 入口,插件生态一员,external 节点不进 live map,完成通知经 `agent.followup` 投普通 user turn 回发起会话)。**红线豁免全部消灭**:「按 sid 冷 resume」从豁免变满足(真机实测:SIGKILL 真 DSH 进程后同 sid `session/load` 原文召回密码)、成本从「永远 —」变真 token 上账、「挤停丢上下文」特殊语义不再需要(挤停后可冷恢复,普通语义)。W0 真机双跑交叉验证(codex 独立复测 + 先行 claude 复测一致)+ K25 物化(`include_bytes!` 内嵌插件字节、per-sid profile 免 pnpm 幂等物化、上游 `dsh-base` 靠产品 CLI 自愈)+ D19 安全修复(bearer 读一次即从 `process.env` 自净,防 DSH 自身 bash 工具子进程继承泄漏)+ K23 凭据阶梯(env 赢,否则逐字节镜像 `~/.dsh/{.credentials.yaml,settings.yaml}` 二件套,零污染实测确认)。全链路真机 DoD 双模式 PASS(方式一冷 resume + 真 token;方式二委派树挂对 + external 节点确认不在 live map),留痕 `docs-local/versions/v0-9-15/w0-logs/`。上一版 = v0.9.14(身份每进程一个 + external node 账本模型),细节见 `.loop/history.md`。:五 vendor 全局配置里那个 admin web token = 「这台机器上所有进程共用一个身份」—— 手起的 agent 因此当不了委派父、也没有自己的 project。三层改:① 全局注册改写 **enrollment credential**(`ccteam-enroll:<id>:<secret>`,只说明「这份配置是谁的」、自身零授权;legacy `Bearer ccteam:<hex>` 一律读作未注册 → daemon 幂等修复,无迁移码)· ② **per-process 身份在 `POST /mcp` 的 `initialize` 签发**(Streamable HTTP 自带的 `Mcp-Session-Id`,五 vendor 真机全验回显;开 binding → 铸 ledger 节点 → mint per-session principal 并 promote,故下游零新路径:手起 agent 的 spawn 走既有 Ambient 路、子会话真挂它下面;id 不是凭据,必须与 credential 同时出示)· ③ **enrolled client = 账本一行**(`managed_by: external`,同一 sid 命名空间/委派树/meta 形状),但**不进 gateway live map** —— 挤停/预算/工具面/事件泵都迭代那张表,故「ccteam 停不了它」由构造保证。**MCP 数据面无 admin 层**:web token 在 `/mcp` 401(它是控制面 `/api/v1` 的凭据),stdio `internal mcp-serve` 连同 1.6k 行整体删除(还活着的命令 = 陈旧配置仍能调用的第二条身份路径);**猜测被删除**:凭据没钉 project 的 caller 必须**自己点名** workspace(`can_see_project` 判,不可见与不存在逐字节一致),daemon 默认项目 / scratch 兜底整条拆掉;console 的复制按钮改发**项目域凭据**(路由形状即 ACL,片段由五个**真** writer 跑出来再回读,永不再把 operator 自己的登录 token 贴进配置)。**身份仗改在服务端赢**:受管 ACP 会话的 per-session 凭据只是**给 vendor 的一个 offer**(实测 grok 4/4 装载自己同名 config 条目,`--plugin-dir` 遮蔽先例不转移)→ `/mcp` 改按**进程血缘**认人(adapter 在第一个握手字节前记子 pid,loopback peer 经 `/proc` 上溯到受管会话),两条路都没到才发 `identity_degraded`(只广播不入 chat、永不致命);同族拔掉自伤:`prime_tool_surface` 的 `mcp_reconnect` 让 claude 无视 `--strict-mcp-config` 重挂全局条目(实测 8/8 tool call 落 enroll tier),rebuild 整体退为 `RespawnRequired`。**读路径也是可重建资源**:`HarnessAdapter::events` 契约化为可重复调用(`event_attachment()` 无默认实现),泵退避重连并两端记账,连接死时敞着的 turn 走**同一条**失败路径落账 + 通知父会话(codex `turn/completed` 不再默认判成功);**活动判定收敛单一解析器**(`classify_session_activity` 折叠文件判据与 daemon 在飞 turn,单调不下调)+ `read_jsonl` 按**行**容错(一个撕裂字节曾让 120 MB 账本整份读空 → 全项目秒读 idle、成本 $0);受管会话 MCP endpoint 一次解析四种方言(pi 的 URL 不再靠祖先进程 env);团队页三处空字段各修其根(`observed_model` 只读展示 / 停止会话读盘解析 / 无价表 vendor 回落 token 数,绝不假 $0.00)。上一版 = v0.9.13(Pi 第六 vendor + paneless turn 全生命周期记账),细节见 `.loop/history.md`。 |
