...
| `ccteam__chat_send_input`(F108 + F114)| `chat_` | **F147 pending STUB**(返回 `{ok:false, error:"NotImplemented"}`)— 把 `TurnInput::UserText` 推给 chat-mode bot;F147 真实现走 `tmux send-keys -l` 直送 + Enter | `{slug: string, bot: string, input: string}` | `{ok: false, error: "NotImplemented", ...}`(F147 后:`{ok: bool, slug, bot, turn_id, turns_jsonl}`)|
| `ccteam__chat_session_reset`(F118)| `chat_` | **F147 pending STUB** — chat session 失效后从 `turns.jsonl` tail 重建 context | `{slug: string, bot: string, preserve_history?: boolean}` | `{ok: false, error: "NotImplemented", ...}` |
| `ccteam__chat_show_turn_log`(F118)| `chat_` | **F147 pending STUB** — 读 `turns.jsonl` tail | `{slug: string, bot: string, last_n?: integer}` | `{ok: false, error: "NotImplemented", ...}` |
