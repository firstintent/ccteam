...
| **marketplace** | `GET /marketplace`、`GET /marketplace/{id}/body`(install 前预览)、`GET /projects/{slug}/marketplace`(带 per-project `installed_status`)、`POST /projects/{slug}/marketplace/install`(`routes/marketplace.rs`) | ccteam-hub 插件市场(track-upstream,详 §6.8):GET 读 hub `index.json`(经 `~/.ccteam/hub-cache/`,`?refresh=true` 强刷);install 从条目 `upstream` 拉内容(host 白名单 + sha256 校验)写项目 `.claude/{agents,skills}/`(多文件 skill 整目录)+ `force` 覆盖 |
...
把 role/agent/skill/workflow 的**内容**从 ccteam repo 里彻底搬走(repo 零提示词类型插件,唯一例外引擎自带 `cto_role.md`),换成一个 **curated marketplace + track-upstream ingestion** 的三角。**v0.8.12 起从「vendor 拷贝每个文件进 hub」改成「跟踪 upstream 仓库」**:hub 只存指针,内容在**安装时**从上游 raw URL 拉进用户项目(顺带让**多文件 / 目录型 skill** 天然可装 —— 正是旧 vendor-copy 模型卡住的地方)。
...
- **install 进用户项目**:从该 plugin 的 **`upstream` URL** 拉内容(**非** hub base),**host 白名单**(只 `raw.githubusercontent.com` + loopback,表外 host → `HubError::HostNotAllowed`,拉前就拒)+ 落盘前 **sha256 校验**(对账 `content_sha`,防篡改/半截)。**单文件** agent / SKILL.md-only skill:写 `.claude/agents/<id>.md` / `.claude/skills/<id>/SKILL.md`,复用 `write_role`/`write_skill`。**多文件 skill**(有 `manifest`):每文件 URL = `upstream` 的 dir + `relpath`,**整批 fetch+verify 后再落盘**(中途 sha 失败不留半成品)到 `.claude/skills/<id>/<relpath>`(`ccteam_core::write_skill_file`,relpath 防穿越)。`installed_status` 对多文件 skill 按 manifest 整目录比对(全齐+匹配=Installed,缺/差=UpdateAvailable)。`force` 才覆盖。
- **两条用户入口**:① CLI `ccteam role search` / `role add`;② web 插件市场浏览器(`MarketplaceView.tsx`):`GET /api/v1/marketplace`、`GET …/{id}/body` 预览正文(install 前 review)、`GET /api/v1/projects/{slug}/marketplace`(带 per-project `installed_status`)、`POST …/marketplace/install` 一键装。

**代码归属**:`crates/ccteam-im/src/hub.rs`(catalog load + cache + upstream-fetch + 多文件 install,主逻辑)+ `crates/ccteam-core/src/{hub.rs,admin_actions.rs}`(core 侧 raw-base / `skill_dir_path` / `write_skill_file`)+ `crates/ccteam-web/src/routes/marketplace.rs`(4 个 REST 路由,挂进 `/api/v1` OpenApiRouter、自动 web-token 门)。ccteam-hub 侧:`scripts/sync.py` + `sources.json` + `index.json`。**红线**:内容 verbatim(不改写)、content_sha 校验(完整性)、host 白名单(只已登记源 host)、ccteam repo 零提示词内容(§三 唯一例外 cto)。
...
| web 附件 + 技能附加(composer ＋ 菜单;vendor 通用)| 两步流:`POST /projects/{slug}/uploads`(raw body → `<project>/.ccteam/uploads/`,25 MiB 上限,远程 host 项目可读拒绝)+ `GET /projects/{slug}/skills`(`.claude/skills/*/SKILL.md` 清单)= `crates/ccteam-web/src/routes/uploads.rs`;turn 面 `TurnForm.attachments[]`(校验:file/image 钉死 uploads 目录、skill id 字符集 + 已装;空 text 有附件即合法;远程会话可读拒绝)+ 组文 `build_turn_text_with_attachments` = `sessions_api.rs` —— 复用 IM 同一行语法 `[attachment image_path|file_path="…"]`(共享助手 `attachment_line`/`attachment_path_key` = `ccteam-im/src/transport/mod.rs`,`wrap_inbound` 同源);skill 行按 vendor 渲染于**单点缝** `skill_attachment_line(vendor,…)`(wire 恒中立 `{kind:"skill",name}`):claude → 点名原生 Skill 工具 `/name`、codex → 嵌 plaintext mention `$name`(其 `TOOL_MENTION_SIGIL='$'`)、grok/opencode/kimi(ACP 无原生 loader)→「read SKILL.md and follow」兜底;读侧 `ccteam_core::list_skills` = `ccteam-core/src/roles.rs`;SPA = `components/ChatComposer.tsx`(＋菜单/chips/拖拽/粘贴/上传中阻发)+ `lib/attachmentsApi.ts` + `submitTurn(attachments)` | `cargo test -p ccteam-web --test sessions_api_test`(upload→turn 组文/skill/拒绝面 4 个 e2e);`cargo test -p ccteam-core --lib roles`;vitest `attachmentsApi`/`ChatComposer` |
...
| **(v0.9.3)四 vendor 全局 MCP 对称注册**(任何 vendor 主会话可当 orchestrator) | writer = `ccteam-core/src/mcp_register.rs`(`install_{grok,opencode}_mcp_into` + `resolve_*_config_path` + `*_registered`;Grok=`~/.grok/config.toml` `[mcp_servers.ccteam]{url,enabled,headers}`、OpenCode=`~/.config/opencode/opencode.json` 运行时 v1 形状 `mcp.ccteam{type:remote,url,headers,enabled}`,均 0600)· 装载 = `ccteam-cli/src/mcp_serve.rs`(`install_{grok,opencode}_mcp`)+ `commands.rs`(config mcp 四 vendor 报告)+ `ccteam-web/src/routes/hosts.rs`(register-mcp)。托管会话不受全局条目遮蔽:Claude strict / Codex deep-merge / OpenCode `MCP.add` 按名覆盖 / **Grok 同名去重 ACP 注入胜(真机探针 2026-07-15:双 server 同连、tools/call 落 session bearer)** | `cargo test -p ccteam-core --lib mcp_register`;真机 `grok mcp doctor`(8 tools) |
...
| **目录三源 + routing notes 透传 + spawn 失败发现面**(目录/观点层 + spawn 面)| catalog 三源并列不抹平(runtime last-seen ⋈ hub `models.json` ⋈ 用户别名注释,各标来源;advisory,**永不当 spawn allowlist**,stale/缺失不阻断任何 spawn)= `crates/ccteam-im/src/{mcp,hub.rs}` · routing notes(`~/.ccteam/routing.md` + `routing/projects/<slug>.md`,dumb markdown,限幅 + source/sha/truncated 包裹后随 `status` 原文透传,只搬运不解释;项目级 > 全局 > hub 默认指南)= `crates/ccteam-im/src/mcp/` · spawn 失败发现面(vendor 未装 → mint sid 前快速失败,错误附同 host 已装 vendor 集 + freshness;host offline 报 offline 绝不本地 fallback;auth unknown / model 不在 catalog 不提前阻断,vendor 原始错误保留)= `crates/ccteam-im/src/{gateway.rs,mcp/dispatch.rs}` | `cargo test -p ccteam-im`(spawn 失败/notes 透传) |
