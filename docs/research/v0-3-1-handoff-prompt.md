...
现成的 `docs/v0-3-1/{prd.md,dev-plan.md}` 节奏跑剩余 4 个 PR,之后 ship V0.3.1。
...
1. `CLAUDE.md` — 整文件(137 行,5 分钟)。重点:
   ...
   - §七 Rust fmt 约定
2. `docs/v0-3-1/prd.md`(1254 行,关键章节按 finding 跳读):
...
3. `docs/v0-3-1/dev-plan.md`(599 行):
   - §1.1 依赖图(上面贴了一份)
...
4. 历史归档(只在改架构红线 / 团队 yaml schema 时翻):
   - `docs/v0-2-2/prd.md` — V0.2.2 patch 模板(本次 V0.3.1 follow 它)
   - `docs/v0-3/prd.md` — V0.3 PRD(M5.0-M5.4 ship 实情)
...
3. **agent 完事 → 你 review**:
   ...
   - 红线 grep:`git grep -nE '\bcct\b' -- 'crates/' 'README.md' 'CLAUDE.md' 'docs/tech-design.md' 'docs/interfaces.md' ':!docs/v0-2-2/' ':!docs/v0-1/' ':!docs/v0-2/' ':!docs/dev-coupling-audit.md'` 应该返回 0(post-F44 后 forward-looking 不应有 `cct` 二进制名)
...
strictly the §F<##> finding documented in `docs/v0-3-1/prd.md` §<N>.
...
## 1. Required reading
- `CLAUDE.md` §三 + §五 + §六 + §七
- `docs/v0-3-1/prd.md` §<N>(F<##> 全文)
- `docs/v0-3-1/dev-plan.md` §<N>(任务列表 + 验收)
- `docs/v0-3-1/dev-plan.md` §8.1(通用前置 — 已知)
...
(详 `docs/v0-3-1/dev-plan.md` §4-§7;以下只列高密度要点)
...
- `docs/v0-3-1/e2e-retro.md` 落档(模仿 `docs/v0-2-2/e2e-retro.md` 模板 / 4-suite 验证)
- `docs/v0-2/README.md` "V0.3.1 起始 (drafting)" → "V0.3.1 ship"
...
- **`cct` 二进制名是 F39 旧约定,F44 反向回滚,二进制名一律 `ccteam`**:新 / 改的代码 / 文档 / 模板里出现 `cct ` / `cct-<某 skill 名>` 都要替成 `ccteam ` / `ccteam-<...>`(`ccteam-control` / `ccteam-team-author` / `ccteam-project-creator` 三个 skill 名是 ccteam-prefix);唯一例外是 `LEGACY_SKILL_NAMES` const + 历史 doc(`docs/v0-2-2/` / F39 / F44 段落)留 `cct-*` 描述历史
...
- `docs/v0-3-1/e2e-retro.md` 存在
- `docs/v0-2/README.md` "V0.3.1 ship" 字样
...
# 任务清单从 docs/v0-3-1/dev-plan.md §4 复制(F48 部分)
