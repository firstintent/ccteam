...
| **(v0.9.0 W1)AgentPrincipal 调度门泛化 + 四 vendor 身份注入** | principal 认证 = `ccteam-im/src/gateway.rs`(`verify_session_principal(sid,secret)→CallerCtx{sid,slug,role}`,去 cto-only)· 门 = `mcp/dispatch.rs`(`execute_session_tool` 按 principal + `_caller_slug` 服务端覆写)· HTTP sid-bearer = `ccteam-web/src/routes/mcp.rs`(注入四件套)· MCP 注入 = `execution/mcp_config.rs`(`acp_mcp_servers_http` 共享 opencode+grok · `codex_thread_mcp_config{,_at}` = `thread/start|resume.config.mcp_servers` HTTP per-thread)· Codex 全局 HTTP writer/旧 stdio 识别 = `ccteam-core/src/mcp_register.rs` + `ccteam-cli/src/mcp_serve.rs`· codex `vendor_uuid` 落盘 + resume-first = `execution/codex_app_server.rs`· `session_spawn` 扩参 = `mcp/protocol.rs` | `cargo test -p ccteam-web --test mcp_session_bearer_test`(含 fake 组合 + ignored 真机 Codex 0.144.x);`-p ccteam-harness --test codex_app_server_test`/`opencode_acp_test`/`grok_acp_test` |
