...
- **`session_list`** — the live roster (busy/idle, `context_pct`, cost/tokens, `parent_sid`), most recently active first; pass `tree:true` for the delegation tree over the filtered rows. Accepts `{project?, activity?, limit?, tree?}` (default cap 30 rows; `truncated`/`total` appear only when the cap bit; null/empty fields are omitted) so a big fleet never floods your context. The web team view renders the same graph live.
