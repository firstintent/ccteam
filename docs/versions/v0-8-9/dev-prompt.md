...
### Phase 3 — ccteam 引擎仓:web UI 整体改造(最大前台)
...
## 二、依赖序(关键)
Phase 0(清 legacy/死链,**留 catalog**)→ Phase 1(hub 填充 + ingestion)→ Phase 2(ccteam 读 hub + 装逻辑,**就位后删 catalog + 旧 role-import**)→ Phase 3(UI,市场浏览器接 Phase 2 API)→ Phase 4(文档+版本)。**Phase 1 在 hub 仓,其余在 ccteam 仓。**
