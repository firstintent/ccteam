...
- **状态**:完成(0a0b26f) · **冲突域**:`crates/ccteam-im/src/mcp + crates/ccteam-cli + crates/ccteam-core(config/projects) + crates/ccteam-web(SPA 文案/tests) + docs` · **建议入口**:codex 委派(规划发卡 + review;**域串行**:与 HERM-1A 同前缀 `crates/ccteam-im`+`ccteam-cli`,等其收口后接续派 s153)
...
- **验证**:实现 `0a0b26f`。tests-first 红态实锤:beacon 字面仍为旧名、spawn 响应缺 `project_source`、多项目缺省仍报 missing-project;实现后字面锁+旧名 unknown tool+新名/status 字节等价全绿。默认梯定向覆盖 explicit/principal/cwd/sole/configured/scratch 六来源、configured 命中、无效 configured 静默跳过、scratch 首用以 `default2` 避让冲突并 scaffold+注册、二次按路径复用同 slug;scratch 独有静态 `note`,其余层无 note,tenant 点名/sole-visible/ACL 语义不变。`cargo fmt --all -- --check` 绿;`make check` clippy 0 warning;`make test-baseline` **1668/0**(锚 1667/0,净 +1 = 新增 `admin_spawn_project_source_covers_explicit_cwd_principal_and_configured`;其余为测试改写);`make web-check` **42 files/422 tests** 绿;doctor live-surface 定向测试 **8 tools/0 STUB** 绿;README/docs/crates 旧 alias 字面 grep **0**(AGENTS 按卡面留规划收口);`writeback.sh` 绿。

### STATUS-SLIM-1 status 返回省 token:退役字段全链清除(owner 直驱 2026-07-26「不能变 token 刺客;功能优先、省 token 其次;从多层通用性对称性修」)
- **状态**:完成(421e316) · **冲突域**:`crates/ccteam-core(state/queries) + crates/ccteam-im/src/mcp + crates/ccteam-cli(status render) + crates/ccteam-web(如有引用) + docs` · **建议入口**:codex 委派(规划发卡 + review;域串行:排 MCP-DX-3 后同 s153)
...
- **验证**:安全子集实现 `421e316`。tests-first 红态:status 顶层仍为 `{projects,orchestrator}` 且项目行含 9 字段;畸形 legacy `phase_state` 仍进入 enum 严格解析而失败。绿态:admin+tenant 共享 renderer 的项目键集恰为 `{slug,cost_24h_usd}`,daemon 键集恰为 `{status,message}`,MCP `orchestrator/socket/reason` 与 7 个死项目键反断言全绿;beacon/status 字节等价绿;legacy `phase_state` 任意旧形状可忽略加载。`PhaseState`/`.phase_state`/`MAX_CONCURRENT_PROJECTS` 生产 grep 0。caller graph:`phase_state` 仅陈旧 resume 写+展示,已退役;两处 concurrency const 无消费者,已退役;`age_seconds` 仅出 MCP、已删且底层仍供 CLI 使用。fmt 绿;`make check` clippy 0;`make test-baseline` 第二轮 **1669/0**(锚 1668/0,净 +1 = 新增 MCP exact-key-set lib test;legacy-state integration test不在 baseline;既有 action test仅改名,±0)。首轮唯一红 `turn_answer_carries_context_echo_for_focused_im_session` 未改代码即定向转绿、第二轮全绿。`make web-check` **42 files/422 tests** 绿;`writeback.sh` 绿。
- **偏差**:按卡面「有活 product logic 即 STOP 该字段」执行,未强删 `ProjectState.team/current_phase/tmux_session`:① `team` 被 init refresh 改写、migration 折入 catalog、web API/watchdog 读取;② `current_phase` 进入 watchdog 告警 message/details;③ `tmux_session` 是 `core::tmux`、CLI attach/peek、project PTY websocket 与 workflow session detail 的运行时路由。故「四字段全退役+全仓 residue=0」未达,卡不能诚实标完成。最窄解锁提议:owner/规划二选一——(A) 重划本卡为 MCP wire slimming+`phase_state` 清除并据 `421e316` 完成;(B) 另批三条迁家/退役:team 统一改读 catalog、watchdog 去 phase 文案、project 级 terminal/PTY 路由改 per-session meta 或整体退役,再删三字段。
- **裁决(规划 2026-07-26)**:**选 A,卡按 wire 瘦身完成**——owner 痛点 = MCP 响应烧调用方 token,`421e316` 已 100% 达成(死载荷全部出 wire,projects 9 键→2 键 + orchestrator 块删除);三活字段已不上 wire,再深挖须动活逻辑(CLI attach/PTY 路由/watchdog 文案),违背卡面钉死的「功能优先」。B 的三条迁家路径立候选卡 STATE-CULL-1(无实现授权;`tmux_session` 项挂 terminal 协议退役门)。原规格 A 段「全仓 residue=0」判定为规划发卡时过度圈定,偏差申报机制按设计工作。

### ACCESS-IA-1 接入页宽屏改版 + 开发者 API 接入卡(owner 直驱 2026-07-27;设计 = 规划定稿,施工照卡)
- **状态**:完成(e2e7850) · **冲突域**:`crates/ccteam-web/web`(AccessView/SettingsPage/SettingsView/HostsView(JoinCard 壳)/lib/i18n/index.css/tests;**纯 SPA 零 Rust**) · **建议入口**:codex 委派(规划发卡 + review)
- **背景**:owner 三点:① 接入页(设置→接入,admin-only)现为 760px 窄单列,「IM 凭据 · Credentials」内嵌整个 SettingsPage(Telegram 卡 + Lark 长表单)吃掉大量垂直空间、表单不对称;② 加「开发者 API」接入面:外部程序直调 REST 建会话/发消息/收回复 —— **纯展示卡,后端已有**(`POST /api/v1/projects/{slug}/sessions`(201 `{sid}`)、`POST /api/v1/sessions/{sid}/turn`(202 异步)、`GET /api/v1/sessions/{sid}/events`(SSE)、OpenAPI `/api/v1/openapi.json` + Scalar `/api/docs`、bearer 与 web 同源 `Authorization: Bearer ccteam:<hex>`);③ 各接入方式重排序。
- **规格(设计定稿)**:A. **宽屏**:`SettingsView` 在 `active==="access"` 时给 `set-detail-inner` 加 `access-wide`(同 `ops-wide` 模式);CSS 新增 `.set-detail-inner.access-wide{width:100%;max-width:1200px}`。B. **顺序骨架 = 人→程序→机器**,三分组 eyebrow 小标(人 · People / 程序 · Programs / 机器 · Machines,i18n 双语),每组 `grid gap-4 lg:grid-cols-2`,full-width 卡 `lg:col-span-2`:1)人 = Telegram|Lark 同排双子 + 用户 Web 登录 full-width 扁条(transport_warning 横幅在本组 grid 上方 full-width);2)程序 = 外部 Agent (MCP)|**开发者 API (REST)(新)**双 code 卡对称;3)机器 = 卫星节点 full-width(JoinCard 加 `bare` 变体嵌统一 Card 壳,HostsView 既有独立观感不破)。C. **IM 卡压缩(垂直主刀)**:Telegram/Lark 改「摘要态⇄编辑态」—— 已配置默认摘要(header 状态 badge + ReadoutRow 状态行 + 「重置」按钮),点击就地展开表单、可取消收起;未配置默认展开表单;Lark 表单重排 = App ID+Secret 同行 `sm:grid-cols-2`、区域改 segmented 单行(Feishu CN|Lark intl)、allowlist textarea rows=2 全宽、动作右对齐。**行为红线零动**:secret 永不回显/预填(密码框始终空,指纹只作文本)、覆盖两步 confirm、ChatIdCapture 轮询与清理、open_id 发现语义、`getImConfig` 数据流。SettingsPage 的 admin 数据获取(getImConfig/reload/transport_warning)上移 AccessView(或抽 hook);default export 无使用者即删(pre-v1 无 shim;MyImSection/UserManagementSection named 出口不动)。D. **API 卡**(`data-testid="access-api"`):文案「从外部程序直调 REST API——建会话、发消息、收回复;与 web 同一 bearer token」;Base URL 行(`{origin}/api/v1` mono+复制)+ curl 三步 pre(首行 `TOKEN='…'` 用 getToken() 内插真值,与 MCP 卡同策略;① `POST /api/v1/projects/<slug>/sessions -d '{"role":"","vendor":"claude"}'`(注释列 vendor 枚举 claude|codex|grok|opencode|kimi)→ `{"sid":"s42"}`;② `POST /api/v1/sessions/s42/turn -d '{"text":"…"}'`(202 异步);③ `curl -N …/sessions/s42/events`(SSE 收回复))+ 整段复制按钮 + 链接行 `/api/docs`(Scalar,`target="_blank"`)与 `/api/v1/openapi.json`;footer:token 与 web 同源,重置在 账号 页。E. **MCP 卡**结构保留,与 API 卡对称化(pre 统一 `max-h`+overflow-auto、footer 补一句),既有文案不动。F. **LoginLinksCard 扁条化**:用户行改 `grid sm:grid-cols-2 xl:grid-cols-3` chip(@handle+复制),空态一句话指向 管理员 tab 添加用户;逻辑/接口零动。G. **i18n**:全部新增文案 zh+en 双语进 `lib/i18n.ts`;Telegram/Lark 表单既有硬编码中文字面维持现状(全面 i18n 化超范围)。H. **测试**:AccessView.test.tsx 六卡 testid 齐(`settings-telegram`/`settings-lark`/`access-login-links`/`access-mcp`/`access-api`/`access-satellite`)+ 渲染序断言(人→程序→机器 testid 出现次序)+ API 卡 origin/token 内插与 `/api/docs` 链接断言;SettingsPage.test.tsx / SettingsView.test.tsx 同步(含 access-wide);存续元素既有 testid 不改名。
- **DoD**:纯 SPA(须碰 Rust/后端即停手偏差申报);`make web-check` 绿且测试数只增(锚 42 files/422 tests);H 断言在案;红线自查(secret 不回显、fail-closed useMe、theme token only、零新 LLM 面);writeback 绿;两 commit 收口(实现→写回)推 dev。
- **验证**:实现 `e2e7850`。接入页改为 1200px 宽屏并按人→程序→机器三组排布;`AccessView` 接管 masked IM config/reload/transport warning,加载期保留六卡确定性 skeleton,Telegram/Lark 已配置默认摘要、未配置默认空表单,secret 空值/两步覆盖确认/Telegram polling cleanup/Lark open_id 语义未动;新增真实 origin+web token 的三步 REST curl 卡、Scalar/OpenAPI 指针,并对称 MCP code card;LoginLinks chips 与 JoinCard bare 壳落地,新增 zh/en 文案。纯 SPA 9 文件,零 Rust/后端/路由改动;`make web-check` **43 files/434 tests** 全绿(锚 42/422,只增),eslint 0;`npm run build` 绿;`cargo fmt --all -- --check` 绿;`writeback.sh` 绿;theme token/fail-closed admin gate/零 LLM 面自查通过,无偏差。

### WEB-ACL-1 web 权限收敛:admin 专属只剩用户管理面,其余按身份+项目归属放开(owner 直驱 2026-07-27)
- **状态**:完成(1025450) · **冲突域**:`crates/ccteam-web + crates/ccteam-core(tenants)` · **建议入口**:codex 委派(规划发卡 + review)
- **背景**:owner 令:普通用户与管理员的差别只保留「新增用户」(用户管理面);其余 web 功能全放开;权限底层逻辑 = 用户 token 身份 + 项目归属(既有 project-owner ACL / `project_acl_layer` 不动 —— 它就是本令说的「底层逻辑」,不是改动对象)。§三 ACL 红线行「全局·运维面仅 admin」随令修订(owner 本令即签核;AGENTS/tech-design 行文 = 规划收口改,dev 会话勿碰)。**保留 admin-only 的完整清单**:① `/api/v1/users*` 全部(create/list/{id}/link/delete/{id}/im)= 用户管理 + 他人凭据;② `GET|PUT /api/v1/config/im` 五路(全局 bot = admin 自己的 IM 凭据;tenant 的 IM 家 = `/me/im` 自助面,身份对称非功能缺失);③ `POST /hosts/join` caller 特例(admin|host-join)照旧;④ SPA `visibleProtocols` 的 terminal admin 门(协议冻结/规划淘汰,非本令范围)。
- **规格**:A. REST 放开(删 `deny_non_admin`,OpenAPI 描述/403 文档同步):`GET /status`(**tenant 行过滤**:`session_rows` 非 admin 按 `can_see_project` 只留可见项目行,聚合计数/daemon 健康不动)· `GET /hosts` · `GET /hosts/{host}` · `POST /hosts/{host}/register-mcp` · `POST|GET /hosts/join-token` · `GET /skills`(marketplace 装全局 skill 本就走 project ACL,面一致)· `POST /projects/{slug}/mcp-servers`(删 admin 门,只走 project ACL)· `sessions_api` turn 的 global-attachment scope 门。B. token 自助:`POST /me/reset-token` 加 tenant 分支 = core `TenantRegistry::rotate_token(id) -> Option<String>`(`session_secret::mint()` 同源,save 后即时生效 —— auth 每请求重读 registry)+ 返回 `{wire_token:"ccteam:<hex>"}`;admin 路 / auth-disabled 400 不动;core 公共 API 变更按 §六 grep 全 caller。C. SPA:SettingsView ops/access `adminOnly:false` + render 去 isAdmin + `resolveSettingsTab` fallback 统一 `"ops"`(admin tab fail-closed 照旧);AccessView 双形态(用 useMe:admin = 现状;tenant = 人组渲染 `access-my-im` 卡(MyImSection 迁入,Card 壳 `lg:col-span-2`)、**不调 `getImConfig`**、无 transport-warning、无 LoginLinksCard;程序/机器组全员 —— MCP/API 卡 token 已按身份内插);AccountPanel reset-token 按钮全员(两步确认不动)+ MyImSection 迁出(家 = Access tenant 形态);ChatComposer 全局 skill 段/fetch 去 isAdmin 门(`fetchSkillLists` 去参);Sidebar/ChatConsole `showTeam` 门删除(team 数据面已按身份过滤);HomeView 角色 picker 去 isAdmin(v0.8.20 F4 beta 毕业 stable;`!(isNewProject && remote)` 条件保留);WorkflowView MCP register 表单去 isAdmin;WorkflowView/AgentsView 死 isAdmin prop 顺扫。D. 测试:REST — tenant_acl_test 403 环拆分(`/status`+`/hosts` 翻 200,`/config/im`+users 面 403 保持,`/skills` 补 200);hosts_multihost_test tenant 403 断言翻 200(join 403 保持);reset_token_test tenant 轮换(旧 token 立死、新 token 生效);mcp_servers_test tenant 本项目 200 / 他人项目 403;sessions_api_test global attachment tenant 放行;SPA — visibleSettingsItems/resolveSettingsTab 新期望、AccessView tenant 形态(有 my-im/mcp/api/satellite,无 telegram-global/login-links)、AccountPanel/ChatComposer/Sidebar/HomeView/WorkflowView 对应翻转。
- **DoD**:`cargo fmt --all -- --check` 干净;`make check` clippy 0;`make test-baseline` 只增(锚 1669/0);`make web-check` 只增(锚 43 files/434 tests);`writeback.sh` 绿;红线自查(admin tab fail-closed 保持 / secret 永不回显 / 零新 LLM 面 / `project_acl_layer` choke point 不绕);两 commit 收口(实现→写回)推 dev。
- **验证**:实现 `1025450`。tests-first 红态:7 个 SPA 定向文件 **20 红/68 绿**,实锤 ops/access、全局 skills、团队导航、角色 picker、MCP 表单仍受旧 admin 门;绿态同集 **88/88**。REST 定向 6 binaries **39/39**:tenant status/hosts/skills 放行,status session rows 按 project-owner 过滤;join-token 管理放行而 `/hosts/join` 仍 403;tenant token 轮换后旧 token 立即 401、新 token 生效且 admin token 不变;本项目 MCP 写 201、他人项目由既有 `project_acl_layer` 隐匿为 404;global skill list/attach 放行;OpenAPI 路由覆盖绿。`cargo fmt --all -- --check` 绿;`make check` clippy **0 warning**;`make test-baseline` **1670/0**(锚 1669/0,净 +1 = core tenant rotate test;web status helper 不入该口径);`make web-check` **43 files/436 tests**(锚 43/434,净 +2);`npm run build` 绿;`writeback.sh` 绿。admin tab/useMe 继续 fail-closed,五路全局 IM + `/users*` admin 门、host join caller rule、terminal protocol UI 门均保留;IM secrets 无回显,`project_acl_layer` 未改,零新 LLM 面。
...
- **状态**:完成(9537720) · **冲突域**:`crates/ccteam-cli(web_chat_bridge) + crates/ccteam-im(gateway restore) + crates/ccteam-core(roles) + crates/ccteam-harness(transcript_tail)` · **建议入口**:codex 委派(规划发卡 + review)
