...
| 项 | 值 |
|---|---|
| Workspace version | `0.10.3`(**已发布**:owner 2026-08-19「拉最新远程 dev 代码,改版本号,然后 release v0.10.3」——v0.10.1/v0.10.2/v0.10.3 三个开发周期未单独发版,合并为一次 v0.10.3 发布。上一版 = v0.10.0(已发布 2026-08-16)) |
| 本版 headline | **两条主线合一版**:① **DSH(DeepSeek Harness)第七 vendor —— 插件即集成**:DSH 无产品化自动化子命令、无全局 MCP 配置缝、官方 ACP 是刻意残缺的 demo(无 resume、无 tool/thought/usage 上线),但把插件系统(Cordis)当第一公民公共 API。ccteam 顺着这个架构做集成而非趴在残缺 demo 上:**一个 npm 包 `@ccteam/ccteam-client`(原名 `@ccteam/dsh-client`,2026-08-28 改)**(仓内 `plugins/dsh-client/`)双面孔——工具面(受管 + 手起,8 原名 MCP 工具)+ 传输面(仅受管,env 双开关,插件自绑 stdio serve ACP,含 `session/load` 架在 `ctx.agents.resume` 上、富事件、审批、取消)。Rust 侧 `execution/dsh_acp/` 坐共享 `execution/acp/`(kimi/grok 同门形状 + resume 阶梯),零新协议机器。**双形态并列一等**(owner 钦点):方式一 = ccteam 集成 DSH(ccteam web/IM 入口,受管 vendor,进 live map);方式二 = ccteam 作为 DSH 插件(DSH web 入口,插件生态一员,external 节点不进 live map,完成通知经 `agent.followup` 投普通 user turn 回发起会话)。**红线豁免全部消灭**:「按 sid 冷 resume」从豁免变满足(真机实测:SIGKILL 真 DSH 进程后同 sid `session/load` 原文召回密码)、成本从「永远 —」变真 token 上账、「挤停丢上下文」特殊语义不再需要。W0 真机双跑交叉验证 + K25 物化(`include_bytes!` 内嵌插件字节、per-sid profile 免 pnpm 幂等物化)+ D19 安全修复(bearer 读一次即从 `process.env` 自净)+ K23 凭据阶梯(env 赢,否则逐字节镜像 `~/.dsh/{.credentials.yaml,settings.yaml}` 二件套)(2026-08-28 owner 决定:ccteam 面板的 REST token 也进 operator 自己的 profile 行、patch 文件 0600,只有手起 `dsh web` 才需人工粘 token;enrollment 仍不进 operator home)。**ccteam-ui = 整页工作台**(2026-08-28 owner 令重做:团队树 | 原生级会话 | 详情,全部由 DSH primitives + token 重建,原生会话组件不可导入;设计稿住 `docs-local/versions/v0-10-4/ccteam-ui-v2-design.md`)。全链路真机 DoD 双模式 PASS,留痕 `docs-local/versions/v0-9-15/w0-logs/`。② **DSH web 一等公民**:ccteam web 新增顶级「DSH」菜单,daemon 开伴生端口(默认 web 端口 + 1)把每个登录身份代理到它自己的 DSH web 实例——architecture = 伴生端口反代(`/dsh/*` 路径前缀方案已被 W1 真机证伪,DSH 资产绝对路径 + 上游无 base-path)。operator 见真 `~/.dsh`(attach-if-detected `127.0.0.1:3080`,绝不与手起实例双写同一 home);每个租户一份 `$CCTEAM_HOME/runtime/dsh/web/<user>/` 隔离空间(合并式物化,保留用户自装插件层,凭据出厂为空走 DSH 原生 Settings→Models 自助)。SPA 由规划会话亲自设计(owner「codex 审美不行」),operator 专属仅 loopback 展示的「打开原生窗口」按钮。**上线后真机 dogfood 抓出三个跨过全部单测防线的 bug**:①`start_for` 已在跑快捷路径持有 `tokio::sync::Mutex` guard 横跨 `.await` 二次自抢同一把锁,不可重入锁自己等自己,一次点击锁死全部 DSH 请求——回归测试断言「重复 start + 之后 status 仍 5s 内返回」而非「一次成功」;②租户 patch 生成器给已在 `dsh.profile.bundles` 里、自带 patch 层已插入同 id 行的 `@ccteam/ccteam-client`(原名 `@ccteam/dsh-client`,2026-08-28 改) **又 insert 了一次**,Cordis 以 `duplicate loader entry id` 拒绝整个 boot(DSH patch 语义:带 `insert` 是插入,带 `id` 不带 `insert` 是覆写;`config` 须扁平,嵌套进命名空间会让 enrollment 静默失效)——两处测试均改为结构断言而非子串断言,因为 bug 恰是「子串对、结构不对」;③`crypto.randomUUID` 仅安全上下文可用,原生 loopback `dsh web` 从不出事,伴生端口把它带到 LAN 明文 HTTP 就整体失灵——修法是代理层缓冲 `text/html` 响应(资产/RPC 照常流式)、用 `crypto.getRandomValues` 现算真 RFC4122 v4 UUID 注入 `<head>` 最前(原生已有则 no-op)、`Accept-Encoding` 剥除保证响应体恒 identity 编码。上一版 = v0.9.14(身份每进程一个 + external node 账本模型),细节见 `.loop/history.md`。 |
...
| **永不主动 kill 长 session(kill 的是进程,不是会话)** | 会话 = sid + 账本 + 焦点路由,**永不**被 daemon 自行终结;daemon 只管**常驻(residency)**:`resident`(持进程/连接)/ `released`(未持有,下条消息按 sid 冷 resume)/ `stopped`(用户显式,`meta.stopped_at`)/ `detached`(体外活)。**空闲释放 = 主机制**(owner 令 2026-08-29):常驻会话空闲 ≥ harness 的 prompt-cache TTL(`sessions.idle_release_secs` 默认 3600 = claude 1h;`idle_release_by_vendor.<harness>` 覆盖;0 = 永不)即释放进程 —— 资格 = 无在飞 turn ∧ 无 HITL 等待 ∧ 无 outlives-turn 后台任务;**容量挤停 = 次级硬上限**(`sessions.max_live`,LRU,**等待用户决定的会话与新会话父链绝不挤,创建永不因容量失败**);两者走同一条释放路(`session_evicted{reason:idle\|capacity}` progress + lifecycle 广播),**都不删焦点路由**;daemon 重启**不**重生进程(只探 body)。预算例外:`budgets.*.max_cost_usd_per_24h` 触顶 auto-disable。`project stop` / `/stop` / `rm --force` 是用户**显式**命令;`/compact /new` 是合法 turn,`/new` 总铸新 sid |
