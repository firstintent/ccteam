...
/// V0.4.6 F90 — F91 cost SoT preview (stubbed in the F90 worktree).
///
/// **F91 ownership note**: the proper implementation collapses three
/// cost sources (progress.jsonl `agent_done`, live state.json reads,
/// legacy state.cost_used_usd) into a single struct. F91 lives in a
/// sibling worktree. Until F91 lands, this stub mirrors the shape the
/// SPA + F84 budget cap will consume so callers can wire against the
/// stable API surface today.
///
/// Stub semantics:
/// - `cost_24h_usd`: sum of `agent_done.cost_usd` events whose `ts`
///   is within 24h of `now`.
/// - `cost_active_usd`: sum of live `state.json::cost_usd` for every
///   open `agent_spawn`.
/// - `cost_total_usd`: sum of every `agent_done.cost_usd` ever
///   recorded.
/// - `session_count_24h` / `session_count_active`: row counters
///   matching the above two filters.
#[derive(Debug, Clone, Serialize)]
...
/// V0.4.6 F90 stub for F91. See [`CostSummary`] note.
pub fn cost_summary(slug: &str, paths: &CcteamPaths) -> Result<CostSummary> {
    let path = paths.progress_jsonl(slug);
    let events = if path.exists() {
        progress::read_all_events(&path).unwrap_or_default()
    } else {
    ...
    for event in &events {
        ...
        let cost = event
            ...
            .unwrap_or(0.0);
        cost_total_usd += cost;
        if let Some(ts_str) = event.get("ts").and_then(|s| s.as_str()) {
            if let Ok(ts) = chrono::DateTime::parse_from_rfc3339(ts_str) {
                if ts.with_timezone(&Utc) >= cutoff_24h {
                    cost_24h_usd += cost;
                    session_count_24h += 1;
                }
            }
    ...
    let active = active_sessions(slug, paths)?;
    let cost_active_usd: f64 = active.iter().map(|s| s.cost_usd).sum();
    let session_count_active = active.len() as u32;
