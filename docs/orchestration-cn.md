...
3. **在 turn 边界收集** —— 每个子会话转 idle 时完成通知各来一条;还缺的用 `agent_read{sid}` 补(缺席/失败的成员标记出来,绝不 kill)。
   每条完成通知只有一行头 —— `s12 done · codex · turn 7 · ctx 19%`(`⚠` 从 85% 起;失败写 `s12 FAILED (<kind>) …`)—— 后面直接是答案节选(默认 500 字符,`notify:"final"` 是 2000;被截断的节选末尾就是读全文的精确调用 `agent_read{sid,n:1,max_chars}`),再无别的 —— 只送一次、作为独立 turn,哪怕子会话完工时你正在 turn 中;`agent_read` 的名册行、transcript 与内联 `agent` 结果都给数字 `context_pct`,据此决定**复用还是新开**不用再多调一次:上下文还宽裕就继续派给它;接近告警带就为下个任务新开一个、旧的闲置。中途叙述根本不会叫醒你 —— 它只留在子会话自己的账本里 —— 所以话痨子会话不多花你一分钱。子会话比内联 `agent{wait}` 还久时,用 `agent_read{sid,wait:240}` 把这次读挂住到它的 turn 结束(带 `since:<cursor>` 重复调用即可继续向前翻 —— 最旧的未读在前,`remaining` 告诉你还剩几条未读);`agent_read{sid,n:0}` 是零正文的「做完了吗」检查。这些都不是轮询:自己雇的会话以完成通知为准;永远不需要你自己去 tail 它的 `turns.jsonl`。
