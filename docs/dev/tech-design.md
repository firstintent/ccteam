...
| **(v0.10.4)ccteam-ui —— ccteam 核心体验进 DSH 原生 UI(第二插件,原 dsh-team)** | **定位** = 在 DSH 的 UI 里用多种 harness,未来可能整体取代 ccteam web,故一切按 DSH 客户端插件机制构建、零 ccteam-web 移植:`plugins/ccteam-ui/`(host 半 = Cordis 插件 + `/ccteam/api` BFF;client 半 = `src/client/index.tsx` 经 `ctx.slots.inject` 注册 `sidebar.footer.action`/`shell.overlay`/`settings.plugin.item` 三座位,业务状态走 inject face `hooks` 舱由框架合成 selector hook,样式 = lightningcss CSS Modules `[hash]_[local]`(`build/css-plugins.ts`,复刻 DSH 预设;手搓 hash 以数字开头曾让整张样式表失效),上游类型 type-only 引自 npm `next` 0.1.1-rc.2)。**包名规则**:`@ccteam/<product>-<name>`(`@ccteam/ccteam-client` / `@ccteam/ccteam-ui`)—— DSH 插件列表剥 `dsh-` 前缀不剥 `ccteam-`,全称显示。**物化通用解**:`materialize.rs` `CCTEAM_PLUGINS` 表(bundle/package/row_id/cache_ns/required_config_key/tgz)+ 物化器接管 `@ccteam/` scope(表外 bundle/patch 行/scope 链接一律清除,scope 外零改动)。 |
