...
| **session ACL = own-only(多用户软分区档0,v0.8.18)** | `chat_can_access(chat, session) = session.owner == *chat` —— 一个 chat(IM `channel:chat_id` / web)只见/只可 `/use`·`/stop`·`/screen`·`/sessions` **自己**的 session;删了「web-channel 通看」+「同-current-project 互看」(后者**反转 v0.8.13 跨前端按项目共享**,勿再加回)。`ProjectState.owner: Option<String>`(`channel:chat_id`,IM `/newproject` 时记;**显式字段、非路径派生**)。**诚实范围**:同 OS uid 下是**软隔离(UX)、非安全边界**(同 uid 仍可读他人文件/`/proc/<pid>/environ`);真隔离 = per-user OS user/sandbox(deferred);同一用户跨前端(web↔IM)复联走档1(per-user web token,后续 patch)|
