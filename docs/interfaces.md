...
**Daemon durable outbound ledger**:`<ccteam_root>/imd/outbound.jsonl` stores gateway replies before and after each `Channel::send`. Each line is `{ts_ms, id, inbound_id, channel, state: "queued"|"sent"|"failed", message: SendMessage, platform_message_id?, error?}`. This is the daemon-side audit/SoT for IM outbound delivery; transport-specific chat logs remain separate.
