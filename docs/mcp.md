...
- `routing` — which channel a task takes into a **busy** child: `inject` (default) merges it into the turn already running, the same route a human's IM message takes; `queue` gives it a distinct FIFO follow-up turn of its own. Idle child: no difference — the task opens a turn either way. Injection never cancels the running turn, never starts a session, and nothing infers urgency from the task text; two channels, chosen by you, with no automatic merging, replacing or superseding. **The trade-off is real and worth one line:** an injected task is resolved by the boundary of the turn it *joined*, so its completion carries that turn's answer — which on claude has also just been shown the injected line twice (once as a mid-turn queued-command preview, once as the next prompt). Steer a child you are actively correcting; pass `routing:"queue"` for a task whose own answer you intend to act on. ccteam-authored completion notifications are always queued, whatever you pass here.
...
- **`partial` + `in_flight`** — what the turn running RIGHT NOW has said, for the reader whose question is "what is it doing", not "what did it answer". `partial:true` stands alone and means exactly one thing: what follows is **not** an answer. `in_flight` is `{turn_id, narration, text?, truncated?, omitted_chars?, requests?}` — `turn_id` is the **execution** turn (what a request is bound to, not a transcript row id); `requests` names the outstanding tasks that turn is answering, and only those; `narration` says how much could be captured, in the same three words `agent_stop` uses: `recorded`, `empty` (it genuinely has not spoken yet) or `unknown` (this harness cannot report a running turn's narration — *not* silence). `text` is the **tail** of the public narration only — the assistant text a transcript would show, never private reasoning — bounded to 2000 characters by the harness and narrowed further by your `max_chars`, with `omitted_chars` counting everything dropped from the head. It has its own budget, so a partial never costs you a finished answer's row; `n:0` is the natural pairing. Reading one changes nothing: `activity` stays `working`, no request is resolved, and the completion notification still arrives when the turn really ends. Absent = no turn is running. Claude stream-json, codex app-server and the ACP vendors (grok / kimi / opencode / dsh) all answer it; pi's rpc bridge and the frozen `terminal` protocol report `unknown`.
