...
- **harness = agentic CLI 驱动器**:每个一套 `HarnessAdapter`。当前六 vendor:**claude-code**(stream-json 主路,smoke 实证)· **codex**(app-server)· **grok / opencode / kimi**(共享 ACP core 薄壳)· **pi**(`pi --mode rpc` 长驻 stdio,**本机 only**:`AgentVendor::host_execution_scope` = `LocalOnly`,跨机门对每个 vendor 穷举);gemini-cli / 其它逐个 adapter 后续接入。
