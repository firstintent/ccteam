...
**A role library, your way — plus a plugin marketplace.** Because a role is just a Markdown file in `.claude/agents/`, you build your team by dropping in `.md` files — write your own, or install ready-made roles, skills, and workflows from the **ccteam plugin marketplace** (backed by the `ccteam-hub` repo): a curated catalog — **official ccteam plugins are featured first**, then the open-source libraries it tracks like [agency-agents](https://github.com/wshobson/agents) and [mattpocock/skills](https://github.com/mattpocock/skills) (both MIT) — that you browse and one-click-install into a project from the web console, or from the CLI with `ccteam role search` / `ccteam role add`. The catalog tracks each upstream repo at a pinned revision (it stores pointers, not copies) and fetches content at install time; skills can be whole directories of files, and every install is content-integrity checked. The default `cto` is a manager that suggests which role fits the task; you make the call with `/role`.
...
**Harness × provider, best-fit drive.** ccteam abstracts each agentic CLI as a *harness*; the model behind it is the *provider* sub-facet. The primary harness is **claude-code**, which ccteam can drive over two channels (the session's *protocol*): the default **stream-json** channel — a long-lived `claude` process spoken to over a bidirectional NDJSON pipe (lightweight, no terminal, fewer moving parts), used for chat-only sessions; and the **terminal** channel — a long-lived **tmux TUI** session driven with `send-keys` plus transcript tailing and the official hooks, chosen when you want the byte-faithful terminal mirror / attach / screenshot. Either way ccteam never scrapes terminal output and never injects a prompt (role personas are bound with the vendor-native `--agent`). **Codex** is supported on a best-effort basis, and more harnesses (gemini-cli, grok-cli, …) are designed to plug in as adapters. Whatever is actually installed on your `PATH` is reported live by the API's `GET /capabilities`.
...
- **Plugin marketplace** — browse and one-click-install roles, skills, and workflows from a curated catalog (official ccteam plugins featured first, then the open-source libraries it tracks like agency-agents and mattpocock/skills) into any project, with a body preview before you install and a content-integrity check on every install. The catalog tracks each upstream repo at a pinned revision (pointers, not copies); multi-file skills install as a whole directory. Reachable from the web console or the CLI (`ccteam role search` / `add`).
