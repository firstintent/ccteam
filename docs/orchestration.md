...
3. **Collect at the turn boundary** — the completion notification fires as each child goes idle; `session_collect` picks up anything you're still missing (an absent or failed member is noted, never killed).
   Every completion notification carries one metrics line for the child — `ctx 19% · turn 7 · $0.42` (`⚠` from 85%) — and `session_collect` / `session_list` / inline dispatch results carry `context_pct` as a number, so you can decide **reuse vs. fresh session** without another call: keep dispatching to a child while its context is comfortably low; once it nears the warning band, spawn a new one for the next task and let the old one idle. Interim notes carry no status, so they cost nothing extra.
