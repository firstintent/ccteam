...
- **env-flake 族**(live-daemon 宿主才出现,不计入 baseline;干净环境应全绿):
  ...
  `ws_*` · gateway 共享 `/tmp/alpha` 并行污染(perf-v1 收口(2026-08-17)再实锤两名成员:`gateway_resumes_dead_session_on_next_turn`(pending-turn 文件被并行测试先 drain,返回 `pending-drain:s1` 兜底值)与 `gateway_status_shows_real_vendor_resume_uuid` —— 两者都 `Gateway::new(fake,"alpha","/tmp/alpha")` 字面共享路径 + 撞 s1 sid 命名空间,隔离必绿、序列化 baseline 全绿;长线修法 = 这族测试改 tempdir,候选卡未立;v0.10.4 DSH2-RUST 收口(2026-08-25)再扩一名成员:`concurrent_replacement_discards_stale_resume_without_touching_newer_entry`——同模式同病,首轮全量 exit 0、二轮闪红、隔离 3/3 绿,与该波 dsh 物化改动零交集,修法归 TEST-HYG-1)(v0.10.0 ship 实锤扩容一族:`gateway::tests` 里 `turn_answer_carries_context_echo_for_focused_im_session` 与 `turn_answer_context_echo_omits_role_when_roleless` 在 PR #180 CI 连续两次独立触发(每次各一,从未同时红)——后者自己的代码注释已明写病根:`FakeAdapter::events()` 跨身份共享一个 `Notify`,并行跑多个 session 的 pump 可能把唤醒错发给还在等待的另一个测试的 session(**纯测试替身竞态,非生产 bug**);前者是 `context_echo_line` 的 `title` 参数(同 turn 内对 meta.json 的 best-effort 读)与「首条消息自动生成标题」异步写之间的窄窗口序竞态。两者均与本次改动(零 touch `gateway.rs`/`ccteam-im`)无关,本机隔离/全量套件复现不了、CI 独立重跑即绿,判定同族,不改测试/不改 gateway.rs——真正修法是给 `FakeAdapter` per-session `Notify`,留作 dev 会话候选卡)。TEST-HYG-1(2026-08-25 opus maker,`.loop/backlog.md` 已收口)兑现了这条长线修法 + 全族 tempdir 隔离,顺带诊断出一个**新的、不属于任何已知族的独立 flake**:`gateway::tests::delegation_reconcile_folds_missed_backlog_into_one_notification` 隔离跑 ~1/20 失败,pristine `daafbda0` 同样复现(非该卡改动引入),3 轮全并行 baseline 均未触发。maker 判断:碰的是委派 at-least-once 通知契约(§三红线),"修成恰好一条通知"本身是否为正确断言需要对着契约裁决,不能靠改断言蒙混——**故意不动手**,留观登记,候选卡未立。
  v0.10.5 波(2026-08-29)再扩一名 **CI 侧**成员:`ccteam-cli` bins 的 `web_chat_bridge::tests::web_chat_newproject_scaffolds_registers_and_cd_works`(dev `82faddb4` 的 deterministic-baseline job 红:读 `ccteam_home/config.yaml` NotFound;前后提交全绿)—— 病根同「宿主给了它什么」:`web_chat_bridge.rs:200-216` 在**进程内** `set_var("HOME"/"CCTEAM_HOME")`,与同一 bins 进程里按 env 解析根的测试并发;修法 = 注入式根 + env 用例搬 `tests/*.rs` + 守卫测试,已立卡 **CLI-ENVTEST-1**。
  ...
  **同日第二起(同病)**:owner 真实 `~/.local/bin/ccteam`(36 MB static-pie release)在 19:28 被换成 173 MB 动态链接 debug 构建 `0.10.3 (45246897)`——PLUG-1 二轮的 `update --channel npm --binary` 沙箱跑没 pin `PATH`/`CCTEAM_INSTALL_DIR`,梯度第二级 `command -v ccteam` 命中真实安装位并覆盖(梯度本身与 install.sh 一致、无错;错在复现没密封)。在跑的 daemon 是旧进程未受影响;规划从 dev HEAD 重建 release 二进制供 owner 装机。**纪律并入上一条:凡会执行 `ccteam update` / 安装梯度的复现,脚本顶部还要 `export PATH=<sbx>/bin:/usr/bin:/bin`(不含 `~/.local/bin`)+ `CCTEAM_INSTALL_DIR=<sbx>/bin`。**
  **checker/maker 的临时复现脚本必须在脚本文件顶部 `export HOME=… CCTEAM_HOME=…` 并断言 `$HOME` ≠ 真实 home**(2026-08-29 事故:codex checker 一次手写复现漏了沙箱前缀,把 owner 真实 `~/.dsh/profiles/web/cordis.patch.yml` 写成坏 YAML 夹具;brief 里「never touch ~/.dsh」的口头约束挡不住漏前缀 —— 纪律:凡要写文件的复现一律走脚本文件 + 顶部 pin + 前置断言,逐条命令内联 env 不算隔离)。
  **共享 `CARGO_TARGET_DIR` 的并行 worktree 会静默给错数**(v0.10.5 波 2026-08-29 三次实锤:别的 worktree 的元数据让编译报缺字段;新加的 integration 测试在复用的旧二进制里"不存在"(29 跑/30 编);同一分钟 `make test-baseline` 与 `cargo test -p ccteam-cli --bins` 报 104/105;另一次 `dsh_acp_test` 在别的构建在飞时 13 红、隔离连跑 3 次全绿)—— 纪律:**并行会话 ≥ 2 时,裁决/基线数字只认干净环境(CI `check.yml`)或「无其他 cargo 在跑 + `cargo clean -p <workspace crates>` 后」的本地跑**;加了测试计数不动 = 先当污染处理(`cargo clean -p <crate>` / touch 重编)再当缺测试;报告数字时注明当时是否有其他 cargo 在跑。磁盘允许时每 worktree 独立 target 更稳。
