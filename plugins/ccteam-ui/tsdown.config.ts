 ...
 * emitting CJS and wrapping it in banner/intro/footer — the DSH preset's own
 * recipe (references/deepseek-harness/packages/client/tsdown.client.ts:437-567,
 ...
 *      in cordis.patch.yml, else system.ts raises "bundle loaded without
 *      registering".
...
// Extension-bearing specifiers: tsdown loads this config through Node's native
...
const PLUGIN_ID = '@ccteam/ccteam-ui'
 ...
 * Type-only imports are erased before this gate and never reach it, which is
 * how the client contract (src/client/slots.ts) pulls the upstream SlotMap /
...
export default defineConfig([
  ...
  },
  {
    ...
    sourcemap: true,
    clean: false,
    // Exact-match externals only: everything else inlines (clsx included),
    // because a require() the loader table cannot answer throws at
    // materialization.
    deps: {
    ...
    plugins: [purityGate, ...createCssPlugins(PLUGIN_ID, undefined, PACKAGE_ROOT)],
    outputOptions: {
