...
- **核心模型 `chat ⇄ project ⇄ session`**:session 是独立一等实体(持久 sid `s<N>`,单调、扛 daemon 重启、不复用);role / harness(claude|codex|grok|opencode|kimi|pi,`AgentVendor` 可扩展)/ provider(model)/ protocol 都是 session 属性,同一 role 可并存多 session;host 是 project 属性,session 继承。roleless(空 role,裸 vendor 自读项目 `CLAUDE.md`)合法且是默认。
