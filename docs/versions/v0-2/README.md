...
> **已 ship V0.3**(2026-05-10,`docs/versions/v0-3/`):web UI dashboard + SSE +
> 写动作 + token auth(5 milestone M5.0-M5.4,新 crate `ccteam-web`,
> workspace.version 0.3.0,738 测试)。**已 ship V0.3.1**(2026-05-10,
> `docs/versions/v0-3-1/`):战略 pivot — flex team kind + adhoc multi-session +
> HarnessAdapter trait + CodexAdapter stub(F46-F51,workspace.version 0.3.1,
> 833 测试)。下方 "V0.3 deferred 项" 仍 deferred(V0.3
> web UI / V0.3.1 session farm 都是新增主线,不替代列出的 deferred 项;
> V0.4 候选见 [`docs/versions/v0-3/prd.md §10`](../v0-3/prd.md) +
> [`docs/versions/v0-3-1/prd.md §10`](../v0-3-1/prd.md))。
...
- Team 重命名(`dev` → `software-development` 等领域命名;牵扯 state.json / slug 前缀过广)
  ...
  (`docs/versions/v0-2-2/prd.md §9`);`dev` 已经短,V0.2.2 PRD 未列入,仍 deferred
