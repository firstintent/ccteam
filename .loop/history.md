...
- **v0.9.6**(dev,未合 main)多 vendor 编排发现面:status vendor 面板(probe 下沉 core `host_registry`、daemon-aware caller 收敛)+ advisory 模型目录三源(runtime last-seen `model_catalog.rs` / hub `models.json` / 用户注释,零 spawn 校验)+ routing.md 透传 + spawn 失败发现面 · **compare 全链退役**(−1716 行,无 wrapper)+ `is_claude_family`/`model_warning` 删除 · README 吉祥物 logo 入 web(favicon/PWA 全套)· docs 模型路由章;owner 直驱,opus/codex/kimi 三 agent 分工 + fable5 规划 review,基线 lib 1469 / web 325(+3 env-flake)/ vitest 389
