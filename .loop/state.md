...
| MCP `screenshot` 工具删除(wire 契约变更,8→7) | **已签核消耗** —— owner 2026-07-26 IM「删除 mcp__ccteam__screenshot,这个是 tmux 时代的遗留产物」;范围仅 MCP 工具面(web `/screenshot` 路由 + core render + IM `/screen` 不动);§三「不解析终端输出」行 screenshot 提法的后果性措辞随令同步;chat_send_file 同问评估 = 保留(IM 外发通道,报告 owner) |
