...
**待下轮 PRD 决断的关键分叉**:① ccteam-per-sandbox(每租户一个完整 ccteam,上面另起薄控制面)vs ② ccteam-as-control-plane(网关居集群入口,session spawn 进 k8s Sandbox,`backend: k8s-agent-sandbox` 作第三 spawn backend)。②保留壳的全部结构位置(网关唯一汇合点 / sid 寻址 / IM 单入口),①把控制面问题推给未知新层(且 N 个 daemon/bot/web 面运维爆炸)—— 倾向②,owner 拍板。②的版本叙事是连续的两次解耦:**v0.8.11 把会话协议从 PTY 解耦(stream-json adapter),v0.9 把会话宿主从本机解耦(`backend: k8s-agent-sandbox` 第三 spawn backend)** —— 网关/sid/IM 命令面/双 SoT 原样复用,新写量 ≈ SandboxClaim 建删 + 镜像模板 + 凭证注入(#3 顺带兑现)+ 出站事件回传流 + sid↔CR 映射与 pause/resume 对接(空闲释放/resume-by-sid 语义委托给 Sandbox 原语)。

**②的租户拓扑与 CLI 落位**(owner 追问澄清,TG 1111-1112):每租户 = 1 个 `ccteam-gw` sandbox(daemon:IM bot/web/REST/账本/sid 注册表/SandboxClaim 编排,**不装 vendor CLI**)+ N 个会话 sandbox(镜像预装 claude/codex + 项目持久卷 + 该客户凭证)。**多租户切在 k8s namespace 层,ccteam 本身永远单租户**;ccteam-gw 自身也是一个 Sandbox CR(agent-sandbox 本来就为"稳定身份+持久卷的有状态单 pod"设计)→ 与「ccteam 跑在单租户独享 sandbox」字面自洽。会话 sandbox 内一个极薄 runner(= #9 的 satellite:出站 WS,零入站)exec vendor CLI;stream-json 的 NDJSON 经 WS 回 daemon,**0.8.11 adapter 消费端一行不改,只是管道从本机 pipe 换成 WS**;codex 同位同法。transcript + progress/turns 双 SoT 落会话 sandbox 持久卷 → resume 在 sandbox 内闭环;空闲两档:pause(整 pod 冻结,in-flight 原地活,恢复即续)/ 深度休眠(pod 回收、卷留存,唤醒后 runner 重 exec `--resume`)。terminal backend session 在 sandbox 内跑 tmux,字节流走同一 WS 回 web 终端,screenshot 同理。分工:gw = 大脑与总机,会话 sandbox = 隔离工位,CLI 坐工位,runner 是传话筒。

**与自托管单机形态的统一**(owner 追问,TG 1113-1114):session = **协议 adapter × 宿主 backend** 两正交轴 —— 协议轴(v0.8.11 立):tmux-PTY | stream-json;宿主轴(新立):**local | satellite(裸机出站 WS)| k8s-agent-sandbox**。网关/sid/IM 命令面/web/双 SoT 对所有组合不变。统一缝 = 一个 **SessionHost 契约**(spawn argv / stdin / 事件流 / 终端字节流 / pause-resume-kill / 健康),三实现一套代码:LocalHost = runner 退化形态(in-process 无 WS)、SatelliteHost = `ccteam satellite` 子命令(任何机器)、K8sSandboxHost = 同一 satellite 二进制在 pod 内当 runner、生命周期归 SandboxClaim;daemon 消费端对三者无感知。差异按 facet 诚实暴露:隔离(无/整机/gVisor)、空闲(exit+resume / pause+深度休眠)、终端(本机 pane / 字节流回传)。产品叙事:**自托管单机永远是默认入门形态(zero-config 不变),集群是同一引擎的 scale-out**;单机→集群 = hosts 注册表加一行 + session 多一个 host facet 值。版本路径:0.8.11 落协议轴 → 0.9 落宿主轴(satellite 先行还是直上 k8s = 排期题非架构题,owner 定)。**§6 版本线待下轮 PRD 按本裁定重排**(信任底座的"多客户在一台机"段落让位于集群编排方向)。
