...
| **Inside your coding agent** — Claude, Codex, Grok, OpenCode, Kimi or Pi (this guide's focus) | delegate with one sentence, in plain language — every MCP-connected session already knows the team tools; nothing to install |
...
There is nothing to install for orchestration itself: `ccteam config mcp` (one-time) registers the ccteam server with **every vendor whose config ccteam may write** — Claude, Codex, Grok, OpenCode, Kimi — and the server's own instructions teach any connected session the delegation flow. Pi is the one exception, by design: ccteam writes none of its config and instead loads its own bridge into the Pi sessions it spawns, so a managed Pi session can delegate while a `pi` you started by hand stays untouched. Want a standing orchestrator persona on top (routing habits and review gates baked in)? Install `team-brain` from the **marketplace** — a persona choice, not a prerequisite. What you do need:
