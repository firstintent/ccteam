...
- **在做**:无 —— **v0.9.11 dev 持续收口**(2026-07-29 owner 拍板当日完成主波 TEAM-1..4(`33545de5`/`9609eb37`/`670e335f`/`e6704daf`)+ sessions_api 封口 `b20e1e96` + docs 双语四份 + ship-gate 治理同步;同日 owner 团队页实测截图追问「vendor 重复/排列乱」→ 查活 daemon 实锤根因(3 条真实登记,含一条离线卫星 smoke-self hostname 撞名残留)→ 追加 TEAM-5 `4c45ed01`(host 反注册 REST+CLI)+ TEAM-6 `61692685`(名册按主机分组+在线离线+移除);`smoke-self` 残留已从活 daemon 注册表清除(备份 /tmp,复核无复活)。owner 再批三候选(「后选动工,派 subagent opus」)→ TEAM-7 `8ec9cf2e`(名册卡点击过滤拓扑)+ TEAM-8 `ee32b6cd`(离线时长外显+7 天建议清理,`HostSummary.last_heartbeat_unix` additive)+ TEAM-9 `3621e871`(HostsView 收敛动作面,观察面归团队页)+ TEAM-10 `36c5793a`(npm「可更新」提示迁名册,vendorLatest 复得消费者)。**v0.9.11 已发布**(2026-07-29:PR #171 merge commit `efce0196` → main CI 绿 → tag `v0.9.11` → release run 30427416169 success,四平台 tarball + SHA256SUMS,`/releases/latest` 已指向;dev 是 main 祖先,免和解直接续用)。上一发布 = v0.9.10(2026-07-26,main `180e91b`)。队列现势 = TD-SYNC-1 / A2A-W5 / FB-2 / P1-1 / P1-2 / TEST-MACOS-1 + 候选 STATE-CULL-1 / A2A-OBS-1..5(V094 gated)。
- **下一周期(v0.9.11 发布后 dev 累积中,版本号未定)**:①`e6fbef7`+`3be807b`(另一会话:live session 继承 tenant project owner + 和解 main 合并)②**`410647d` ACP 结局契约修复**(owner 2026-07-30 直驱,自真机 kimi s172「STUCK 16m 后答半句」逆推;详见人工门与教训两行)。⚠️ **本周期 dev→main draft PR 尚未开**(AGENTS §五「周期开始即开」未执行)→ `check.yml` 触发面 = `push[main]` + `pull_request[main]`,**推 dev 本身不触发任何 job**,故上述三个提交**零 CI 覆盖**(含其 `test` job = baseline 同口径的干净环境仲裁);基线数字待 PR 开启后回填仲裁值。开 PR 需 owner 发话(`gh pr create` 通路可用)。
...
- **`410647d` 后待仲裁口径 = 1716**(+8 `--lib`:acp `protocol` 5 + `translate` 2 + `ccteam-im gateway` 1;另 +2 集成 `kimi_acp_test` 不在口径内)。**本机 macOS 未跑 `make test-baseline` 全绿声明**(TMPDIR 族在口径内确定性红);已跑实证:`-p ccteam-harness --lib` 392/0 · `-p ccteam-im --lib` 551/0(rebase 后)· `-p ccteam-harness --test kimi_acp_test` 13/0 · clippy `--workspace --all-targets` 0 warning · fmt 干净。**回归证据**:`cargo test --workspace --no-fail-fast` 改动前/后同为 2777/37 且失败集合逐名一致(stash 重跑对照;按教训「stash 对照只证非本 diff 所致」,此处正是该结论的有效射程),37 红全为已登记 macOS 宿主族(codex SUN_LEN / tmux / file-watcher / skills TMPDIR / `ws_*`),无一在改动路径
- `ccteam-web` 全量:**344 通过 + 3 红** = `pty_ws_test` `ws_*` 已登记 env-flake 族(live-daemon 宿主,非回归;sessions_api 两红已于 `b20e1e96` 封口清零)· vitest **532**(50 文件;上口径 436 + TEAM-1..3 +52 + TEAM-6 +18 + TEAM-7..10 +26)· tsc/eslint 干净 · Playwright **7**(未重跑,沿用口径)
...
| 一次性治理写权授予 dev 会话(本次) | **已签核消耗** —— owner 2026-07-30「写治理」;dev 会话据此写 `.loop/state.md` + `.loop/backlog.md`(新卡 ACP-LEDGER-1 / KIMI-UPSTREAM-1)+ `docs/dev/tech-design.md` 指针行。**常态不变**:治理面写权归 Fable 5 规划会话(AGENTS §五),本次为 owner 点名例外,下次仍须停手申报 |
