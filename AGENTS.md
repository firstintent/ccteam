...
| 测试 baseline | 确定性口径 `cargo test --workspace --exclude ccteam-web --lib` = **1367/0**(W1-W4 净增委派/远程/可视化测试);全量口径 live-daemon 宿主有 env-flake 族(`inbound_wiring daemon_*`/`daemon_test register_*`/`im_progress_*`/`codex_streaming_delta`/`resume_*`/`ws_*`/`hook_*` —— 与 live daemon 竞争,干净环境全绿,起手重记);`ccteam-web` 干净 317/3(3 = `ws_*` pipe-pane env-flake);vitest 354(SPA);Playwright 7。干净无 live daemon 宿主应全绿 |
| Clippy | 0 errors + 0 warnings(`cargo clippy --workspace --all-targets -- -D warnings`,含 `ccteam-web`)|
| 当前在做 | **v0.9.0 = Agent2Agent 基础能力(多 harness × 多机中立委派底座),W1-W4 已落 dev(未 tag、未部署)**。铁律:**只做单 harness 做不到的**(跨 vendor 身份/路由/账本/观测 + 跨机执行),**永不做厂商能力**。已落:**W1** AgentPrincipal 通用调用面(`(sid,secret)` 调度门泛化去 cto-only + 四 vendor per-session MCP 身份注入 + codex `thread/start.config.mcp_servers` per-thread 注入 + codex `vendor_uuid` 落盘修重启丢上下文 + opencode/grok resume 补 `mcpServers` + `session_spawn` 扩 vendor/model/effort/protocol/host/title);**W2** 委派语义(child meta `parent_sid`/`delegation_depth`/`title` + 落盘 `delegation.json` watch + pump 完成通知 append→notify 顺序合同 + 启动 reconcile at-least-once + dispatch `wait_seconds`/`notify`/`idempotency_key` + 护栏 depth/children/delegated/cycle + Ambient 预算闸 + `/sessions` 树 + 7 个 `delegation_*` progress 事件)+ **引擎中立化**(废除 cto:删模板/种子,init 后 `.claude/agents/` 空,全链 roleless 默认);**W3** 跨机执行(`ccteam host serve` 卫星 + `ccteam-exec.v1` WS 桥 + claude 远程 stream-json spawn + rebuild 三路径 re-gate 修远程会话本地重生 + registry 0600 + slug gate;codex/opencode/grok 远程 = 显式 NotImplemented);**W4** 团队可视化(`GatewayEventKind::Delegation` 广播 + `/api/v1/agents/{graph,events}` 全局 SSE + SPA 团队泳道图/时间轴/侧栏 + 删 legacy `/sse/*` 死代码 794+ 行 watcher)。**剩 W5**:hub 示例配方 + 三场景真机 smoke + README/usage 重写。**tag/部署 HELD。逐版改动史 = `git log` + `docs-local/versions/v0-9-0/`(gitignored);协议一律以代码为准。** |
...
| **跨机 = host 是 spawn 参数,执行位置是传输层参数** | `session_spawn` 带 `host`;卫星 `ccteam host serve` = 协议无关「进程+管道」宿主(`ccteam-exec.v1` WS,ExecSpec 首帧);主侧把 `(reader,writer)` 从本机 stdio 换成到卫星的 WS 双工,adapter 协议逻辑**零改动**(`spawn_from_io`/`from_halves` 泛型缝)。卫星**自己**解析 binary(vendor→自身 PATH/`CCTEAM_*_BIN`,主侧永不下发路径)+ cwd(slug→自身注册表)+ files 限 `.ccteam/chat/<sid>/` + env 白名单仅 `CCTEAM_*`。**terminal 永不上多机**(硬拒);rebuild 三路径 re-gate host(offline→可读错误,**绝不本地重生**);turns/progress/cost 全记主 daemon;HITL 跨流回主侧批准。join/agent token = 运维登记面非安全边界(LAN 信任模型)。远程 verdict 钉 claude,codex/opencode/grok 远程 = 显式 NotImplemented |
