...
One trade-off worth knowing about the steer: on Claude the steering line is shown to the model twice (once as a mid-turn preview, once as the next prompt), which costs tokens — and it is the turn the CLI **re-runs** it in that answers it, not the turn it joined, whose boundary is not the end while that line is still waiting to be replayed. Steer when you are correcting work you can see; ask for a queued turn when you want a separate, quotable answer to the new task.
...
3. **Collect when each task really ends** — one completion notification per task, at the boundary that ends the work; `agent_read{sid}` picks up anything you're still missing (an absent or failed member is noted, never killed).
   **A Claude child answers in as many turns as its own work needs.** Claude Code wakes its own model: when a background task, a `Monitor`, or an `Agent` the child launched finishes, the CLI prompts it again and the model answers in a fresh turn ccteam never submitted. So a boundary ends your task only when the harness is holding nothing that will wake the child again — no background task left in its snapshot, no injected line the CLI has still to re-run as the next prompt. Until then your request stays bound (the turn the vendor opens next inherits it), collects one `progress` row naming that execution turn — visible in `agent_read`'s request rows — and **nothing is pushed to you**, neither `brief` nor `final`, though every one of those turns is billed. The child reads `working` in between (`stale` after five silent minutes, the ordinary quiet-session word), and an `agent_stop` still cuts the chain wherever it has got to. You get exactly one notification, and it points at the turn that actually carried the answer. On codex and the ACP vendors a turn exists only because ccteam asked for one, so every boundary there is already the end.
