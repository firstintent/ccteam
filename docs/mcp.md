...
- `routing` — which channel a task takes into a **busy** child: `inject` (default) merges it into the turn already running, the same route a human's IM message takes; `queue` gives it a distinct FIFO follow-up turn of its own. Idle child: no difference — the task opens a turn either way. Injection never cancels the running turn, never starts a session, and nothing infers urgency from the task text; two channels, chosen by you, with no automatic merging, replacing or superseding. **The trade-off is real and worth one line:** on claude the injected line is shown to the model twice (once as a mid-turn queued-command preview, once as the next prompt), and it is the turn the CLI **re-runs** it in that resolves the request — not the turn it joined, whose boundary is non-terminal precisely because that line is still waiting to be replayed. Steer a child you are actively correcting; pass `routing:"queue"` for a task whose own answer you intend to act on. ccteam-authored completion notifications are always queued, whatever you pass here.
...
**Every call is a request with an identity.** ccteam mints a `request_id` and makes it durable *before* the vendor is written to, carrying its own parent, `notify` mode, `title` and lifecycle (`accepted` → `queued` | `submitted` → `executing` → `answered` | `failed`). A request is resolved **only** by the execution turn it is bound to — never by recency, timestamp or queue position — so a second dispatch to a busy child cannot inherit the first one's answer, rename it, or redirect its notification. The rule runs both ways: a turn boundary that no request is bound to (the child's own human asked it something; a channel whose event ids the dispatch path never sees, such as the frozen `terminal` protocol) resolves **nothing** — the outstanding requests keep waiting, with delivery `unknown`, rather than being closed by somebody else's answer.

**And only a *terminal* boundary resolves it.** Claude Code wakes its own model: a background task, a `Monitor`, or an `Agent` the child launched finishing makes the CLI prompt it again, and the model answers in a brand-new turn ending in its own `result` that ccteam never submitted. So a `result` ends a request only when the harness holds nothing that will wake the session again — an empty `system:background_tasks_changed` snapshot, and no line ccteam injected mid-turn still waiting to be re-run as the next prompt. A boundary that fails that test records one `progress` entry on every request bound to it (naming that execution turn), keeps every binding — the turn the vendor opens by itself inherits the previous execution turn's — and pushes **no** notification, neither `brief` nor `final`; its tokens are accounted all the same. `activity` stays `working` across the chain (`stale` after five silent minutes), and an `agent_stop` still cuts it: the `interrupted` record settles the bindings. Codex and the ACP vendors have no vendor-originated continuation — a turn there exists only because ccteam prompted for one — so every boundary on them is terminal.

Responses (compact JSON): async → `{sid, request_id, turn_id, status, delivery, queue_position?}`.

- `status` is what the adapter actually **did**, never an echo of `routing`: `started` (the child was idle and this opened a turn), `injected` (it joined the turn already running), or `queued` (it is a distinct follow-up turn waiting its turn — also the shape when the task is queued behind a pre-restart process, which adds a `hint`). A harness with no injection channel safely serves an `inject` as a distinct turn and reports `queued`; it never claims `injected` for a line the model has not been shown.
...
- **With `sid` = that session's transcript**. `n` defaults to **1**, selecting the latest turn; repeated calls can return the same answer. For incremental reads, save `cursor` and send `since:<cursor>`: only later turns are eligible, oldest unread first unless you explicitly request `tail:true`. An unknown cursor is an error; it never silently replays the transcript. An empty page preserves a supplied cursor. Reading and the `collected` ledger event do not acknowledge consumption or hide future results. Body: `{activity, context_pct?, cursor?, remaining?, latest?, cost_usd?, tokens_total?, residency?, truncated?, partial?, in_flight?, in_flight_omitted?, requests?, requests_remaining?, resolved_requests?, unknown_requests?, turns:[{turn_id, content, outcome?, error_kind?, error?}]}`. `remaining` counts matching turns withheld by paging or budget; `latest` names the newest turn when the page does not end on it. Empty `turns` can mean no new answer or an insufficient budget; check `remaining`. `activity:"working"` remains working between continuation turns until the last boundary.
- **`max_chars`** defaults to 1000 (100–50000), shared across the serialized JSON values of `turns` + `requests` + `in_flight`, including metadata and escaping. Status, cursors, omission counts and wait-resolution control fields sit outside that budget. Short payloads leave their allocation to longer ones. Turns preserve the conclusion when available, otherwise a head/tail excerpt; if a full read pointer cannot fit, use the returned `turn_id` with `turn` and a larger budget. `truncated:true` reports cut content or diagnostics (`error` / `error_kind`), or rows withheld by the budget. A nonempty page reserves space for at least one turn's intact id and outcome before sharing with requests or narration, so a long error cannot block a forward cursor or an exact read. If immutable identity alone cannot fit, the read explicitly reports the required budget. The full text remains in the ledger.
- **`requests`** — outstanding work in acceptance order, at most ten rows. `history:true` also includes the most recent terminal requests after outstanding ones. Each row is `{request_id, parent_sid, state, notify, title?, queue_position?, turn_id?, answered_turn?, progress?, created_at, delivery}`. `requests_remaining` counts eligible rows withheld by the row cap or shared budget; increase `max_chars` to retrieve larger rows. Request rows and their identities remain intact. `progress` names every non-terminal boundary the request rode through while the child's own background work ran.
- **`partial` + `in_flight`** — the current execution turn's public narration, never an answer. `in_flight` is `{turn_id, narration, text?, truncated?, omitted_chars?, requests?}`; request ids stay intact. `narration` is `recorded`, `empty` (has not spoken), or `unknown` (the harness cannot report it). `text` retains the tail within the shared budget; `omitted_chars` counts all discarded source characters, including the harness's own cap. If even the running turn's metadata cannot fit, `partial:true` + `in_flight_omitted:true` still reports running work. `n:0` leaves more budget for it. Reading resolves no request and does not cancel a completion notification. Claude stream-json, codex app-server and ACP vendors report narration; pi and terminal report `unknown`.
- **`turn:<turn_id>`** — that exact turn, without unrelated request rows or running narration. It cannot be combined with `since` or a nonzero `wait`; an exact read already names an existing answer. A missing turn errors. Excerpt read recipes request `max_chars:50000`, the accepted ceiling; a serialized turn larger than that still reports truncation.
...
- `wait` (with `sid`) — seconds to hold the read open while the target's turn is in flight, 0–240 (default 0). At the boundary you get the ordinary body containing the final turn, plus `resolved_requests`: which of **your** tasks on that session this read answered, named, so an answer is never mistaken for another task's. A task that stopped being resolvable WITHOUT being answered — its turn was cut short by an `agent_stop`, its parent became unreachable, its dispatch never reached the vendor — is named in `unknown_requests` instead: nothing will answer it now, and you are not holding an answer to it. On timeout the ordinary body with `activity:"working"`; with nothing in flight it returns at once. A lapsed wait never touches the turn. `agent_read{sid,wait:240,since:<cursor>}` in a loop is the way to sit out a child that runs longer than an inline `agent{wait}` can cover — never tail its `turns.jsonl` yourself. When the wait returns at a boundary and you are the parent that dispatched the task, the completion notification is suppressed: you already have the answer.
- The retired `limit` parameter is a hard error (renamed `n`).

### `agent_stop` — explicitly end a session

`{sid}` → `{sid, stopped:true, interrupted?, undelivered?, resume_policy?}`. An explicit command, never a proactive kill, and it ends a **process**, not a session: the sid, the transcript and every undelivered line ccteam is holding all survive, and `agent_read{sid}` still reads them.

- `interrupted` — `{turn, exec_turn, reason, narration, requests?}` when a turn was running. `reason` is `stopped` (you ended it) or `body_exited` (the process was already gone — a crash, a vendor death, a body that outlived a daemon); both are cuts and neither is ever reported as a completion. ccteam appends one bounded row to the transcript for it (`outcome:"interrupted"`), so a child stopped after half an hour of work no longer reads back as having said nothing; `turn` is that row's id, for one exact `agent_read{sid,turn:<id>}`. `narration` says how much of it could be captured: `recorded` (the tail of what it was saying, ≤2000 chars), `empty` (it had genuinely not spoken yet), or `unknown` (this harness cannot report an in-flight turn's narration — *not* silence). The requests bound to that turn are marked `interrupted`; a restart never turns that record into a completion notification.
- `undelivered` — the tasks of yours it never ran, each `{request_id, title?, state, delivery, retained_in?}`. `delivery` splits the two claims a stop must keep apart: `undelivered` (confirmed never handed to the vendor) and `unconfirmed` (the bytes were written and nothing was ever observed running them — a stdin flush is not proof the model read anything). `retained_in` names the file still holding the line — `deferred-input.json` (keyed by execution turn) or `pending_turns.jsonl` (keyed by request id) — and its presence is what makes the request *still outstanding*; a row without it is over, and its `state` says which way. Retention is only ever claimed where it can be **proved**: lines ccteam holds that name no request are reported as a bare `retained_unattributed:N`, never attributed to one of your tasks.
- `resume_policy: "replay_after_first_result"` — present only when something is retained. The exact contract: the next `start_thread` of **that same sid** reloads every retained line from `deferred-input.json` and `pending_turns.jsonl` and flushes them after that life's **first** `result` — the message that triggered the resume runs first, as its own turn, then the retained lines in their original order, oldest first. Nothing is dropped, merged, deduplicated or reordered; an explicit stop cancels nothing and ccteam never re-sends on your behalf. The cut turn itself is *not* replayed.
- `settled:false` + `error` — the process stopped but the record of what that cut short could not be written. Everything else in the body still describes what happened in memory; the durable half did not land, so treat the receipt as unproven and check `agent_read{sid}`. An agent may only stop its own descendants — and a hand-started client that reconnects is a *new* ledger node, so its earlier hires are no longer its descendants: stop those from the web console or `POST /api/v1/sessions/{sid}/stop`, which the refusal itself says. (ccteam itself has exactly two automatic brakes: the daily per-vendor budget cap refuses *new* work, and live-session capacity releases the least-recently-active idle session — creation never fails for capacity.)
...
An omitted `notify` **inherits** your most recent choice among still-outstanding requests on that child; an explicit mode overrides, and with no precedent the default is `brief`. Booleans parse (`true`→final, `false`→off); retired `all` errors. Taking an answer through `agent{wait}` or a boundary-returning `agent_read{sid,wait}` suppresses that request's notification; waiting on B does not suppress A. Ordinary reads and `collected` events do not cancel parked notifications, since an excerpt is not consumption confirmation.

Delivery needs a managed parent and uses ordinary user turns. Claude stream-json holds notifications while the parent is busy, then immediately batches the adjacent notifications already queued at the boundary; there is no collection window. Explicit tasks and slash commands remain FIFO barriers. A batch starts `[Notification batch <id>: N items]`, preserves each notification's request id, answer turn, result and exact read reference verbatim, and states `[Notifications still undelivered: R]`. A 50000-character ceiling, including framing and reserved replay marker space, splits a larger queue deterministically across later boundaries. Each request keeps its own ledger receipt. The durable queue survives crashes; replay is marked `[Repeated notification delivery: <id>]`, and identical request id + answer turn identifies the same completion. This is **at-least-once across a crash**, not a consumption acknowledgement. Between processes, pending notifications are delivered on resume.

If one indivisible notification itself exceeds the batch budget, it is delivered alone with `[Oversized indivisible notification: delivered alone; batch budget 50000 chars]`; its bytes are preserved and later items continue at subsequent boundaries.

A hand-started parent has no return transport: dispatch replies and initialization state `notify_deliverable:false`; use `agent{wait}` or `agent_read{sid,wait}`. Dispatching to a session you did not hire is a handoff: first contact runs and is recorded, but subscribes you to nothing unless you explicitly pass `notify`. Once you have named a mode on that peer, follow-ups inherit it.
