...
实际执行：F60 / F61 / F62 / F63 / F69-docs 五路初始并行；F64 在 F63 merge 后并行；F65 / F66 在 F60 merge 后并行；F67 / F68 / F69 顺序串行。F61 因 base 太老需要重做一次（fresh main rebase）。详 `docs/versions/v0-4-0/e2e-retro.md` §2 retro。
