...
3. **Collect at the turn boundary** — the completion notification fires as each child goes idle; `session_collect` picks up anything you're still missing (an absent or failed member is noted, never killed).
   Every completion notification is one header line — `s12 done · turn 7 · ctx 19%` (`⚠` from 85%; `s12 FAILED (<kind>) …` on failure) — followed by the answer excerpt, nothing else; and `session_collect` / `session_list` / inline dispatch results carry `context_pct` as a number, so you can decide **reuse vs. fresh session** without another call: keep dispatching to a child while its context is comfortably low; once it nears the warning band, spawn a new one for the next task and let the old one idle. Interim notes carry no status, so they cost nothing extra.
...
**The bill stays visible.** `session_list` and `session_collect` rows carry the accrued `cost_usd` / `tokens_total` per member, so a fan-out's cost is a sum you can read, not a surprise.
...
- You're in a **plain vendor CLI session** for the config-writable vendors, or in DSH's web UI with `@ccteam/ccteam-client` connected. (Some SDK-driven sessions don't load user-scope MCP config; see §9.)
...
- **`session_list`** — the roster (busy/idle, `context_pct`, cost/tokens, `parent_sid`), most recently active first; pass `tree:true` for the delegation tree over the filtered rows. A row carries `residency` only when ccteam holds no process for it: `released` means the session is real and resumes on your next dispatch (reuse it — do not spawn a duplicate), `stopped` means its user ended it. Accepts `{project?, activity?, limit?, tree?}` (default cap 30 rows; `truncated`/`total` appear only when the cap bit; null/empty fields are omitted) so a big fleet never floods your context. The web team view renders the same graph live.
